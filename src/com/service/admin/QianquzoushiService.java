...
import com.dao.QianquzoushiDAO;
...
@Service
public class QianquzoushiService {

	@Autowired
	private QianquzoushiDAO qianquzoushiDAO;
	...
	public int updateQianquzoushi(Qianquzoushi qianquzoushi){
...
//		int n = hqldao.update(hql, p);
		qianquzoushiDAO.merge(qianquzoushi);
				
		return 1;
	}
	
	/*根据id查询中奖纪录*/
	public List getQianquzoushiById(int id){
		String hql = "from Qianquzoushi where qqzsId = ?";
	...
	public void plShanChu(String kjId){
		String hql = "delete from Qianquzoushi where qqzsId in ("+kjId+")";
		hqldao.bulkUpdate(hql);
	}
	/* 查询所有中奖记录*/
	public Map getAllQianquzoushi(String username, Timestamp mintime,
			Timestamp maxtime) {
	...
	// 查询所有往期开奖
	public Map getAllQianquzoushiByTime(String kjQishu, Timestamp mintime,
			Timestamp maxtime) {
		StringBuffer sb = new StringBuffer();
		List listparam = new ArrayList();
		sb.append(" where 1=1 ");
		...
		if (kjQishu != null && kjQishu.trim().length() > 0) {
			sb.append("and qqzsQishu like ?");
			listparam.add("%" + kjQishu + "%");
		}
		if (mintime != null) {
			sb.append("and qqzsTime>=?");
			listparam.add(mintime);
		}
		if (maxtime != null) {
			sb.append(" and qqzsTime<=?");
		...
		String sumHQL = "select count(*) from Qianquzoushi" + sb.toString();
		List sumList = hqldao.query(sumHQL, listparam.toArray());
		...
		String hql = "from Qianquzoushi" + sb.toString() + " order by qqzsId desc";
		List list = hqldao.query(hql, listparam.toArray());
	...
	public List getMaxQishu() {
		String hql = "select max(k.qqzsQishu) from Qianquzoushi k";
		List list = hqldao.query(hql);
		return list;
	}

	// 球号入库 每期开奖写入数据库
	public void addqiuhao(Qianquzoushi qianquzoushi) {
		Integer a  = qianquzoushi.getQqzsBlueone();
		Integer b  = qianquzoushi.getQqzsBluetwo();
		short hewei = MathUtil.hewei(a,b);
		qianquzoushi.setQqzsBluehewei((int)hewei);//添加和尾
		qianquzoushiDAO.save(qianquzoushi);
	}
