...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 充值管理 <span class="c-gray en">&gt;</span> 所有充值记录 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
<form action="${pageContext.request.contextPath}/rechargerecord.do?p=Allchongzhirecord" method="post" name="f" id="f"  autocomplete="off">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"></span> <span class="r">共有数据：<strong>
		<c:choose>
			<c:when test="${empty chongzhimap.sum}">
				0
			</c:when>
			<c:when test="${not empty chongzhimap.sum}">
				${chongzhimap.sum}
			</c:when>
		</c:choose>
	</strong> 条</span> 
	...
	<table class="table table-border table-bordered table-bg table-sort">
		<thead>
			<tr>
				<th scope="col" colspan="9">充值记录</th>
			</tr>
			<tr class="text-c">
		...
		<c:forEach items="${chongzhimap.list}" var="cz" varStatus="i" >
			<tr class="text-c">
				...
				<fmt:formatNumber value="${cz.rrMoney}" pattern="0.0"></fmt:formatNumber>福贝</span>
				</td>
				<td>${cz.rrDingdanhao}</td>
				<td><fmt:formatDate value="${cz.rrTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>
					<c:choose>
						<c:when test="${not empty cz.rrZfbdingdan}">
							${cz.rrZfbdingdan}
						</c:when>
						<c:when test="${not empty cz.rrWxdingdan}">
							${cz.rrWxdingdan}
						</c:when>
					</c:choose>
				</td>	
				...
				<td class="td-status">
					<c:choose>
						<c:when test="${cz.rrType==0}">
							<span class="label label-success radius">支付宝</span>
						</c:when>
						<c:when test="${cz.rrType==1}">
							<span class="label label-success radius">微信</span>
						</c:when>
						<c:when test="${cz.rrType==2}">
							<span class="label label-success radius">竞猜中奖</span>
						</c:when>
						<c:when test="${cz.rrType==3}">
							<span class="label label-success radius">充VIP送</span>
						</c:when>
					</c:choose>
				</td>
				<td class="td-status">	
					<c:choose>
						<c:when test="${cz.rrStatus==0}">
							<span class="label label-danger radius">失败</span>
						</c:when>
						<c:when test="${cz.rrStatus==1}">
