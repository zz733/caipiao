...
<body>
<form  action="${pageContext.request.contextPath}/goods.do?p=findGoods1&gstatus=0" method="post" >
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 商品管理 <span class="c-gray en">&gt;</span>商品下架 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 价格区间:
		<input type="text" placeholder="输入福贝金额" class="input-text" style="width:120px;" name="begingprice" value="${param.begingprice}">
		-
		<input type="text" placeholder="输入福贝金额" class="input-text" style="width:120px;" name="endgprice" value="${param.endgprice}">
		<input type="text" class="input-text" style="width:250px" placeholder="输入商品名称" id="" name="gname" value="${param.gname }">
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜商品</button>
	</div>
</form>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="plshangjia()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe615;</i> 批量上架</a> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr>
				<th scope="col" colspan="9">商品下架</th>
			</tr>
			<tr class="text-c">
				<th width="25"><input type="checkbox" id="allcheck"></th>
				<th width="70">商品编号</th>
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>${m.gname }</td>
				<td> ${m.gprice }(福贝)</td>
				<td><img src="${m.gimages }" style="width:140px;height: 80px"></td>
				<td><fmt:formatDate value="${m.gend}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td class="td-status">
					<c:choose>
						<c:when test="${m.gstatus==1}">
							<span class="label label- radius">已上架</span>
						</c:when>
						<c:when test="${m.gstatus==0}">
				...
				<td class="td-manage">
					<c:choose>
						<c:when test="${m.gstatus==0}">
							<a style="text-decoration:none" onClick="goods_start(this,'${m.gid}')" title="上架"><i class="Hui-iconfont">&#xe615;</i></a>
						</c:when>
					</c:choose>
...
function goods_start(obj, id){
				...
				layer.msg('该商品已上架！',{icon:1,time:1000});
				$.ajax({
				...
				error:function(data){
						window.location.href="${pageContext.request.contextPath }/goods.do?p=goodsXiajia&gstatua=0";
					},
				});
			});
		}
	...
	function plshangjia()
{
	...
	layer.confirm("确定要上架吗",function(index){
		$.ajax({
		url:"${pageContext.request.contextPath}/goods.do?p=MostupdateGsatus1",
		type:"post",
		data:{'gid':checkboxList.toString(),},
		success:function(){
			layer.msg("批量上架成功!",{icon:6,time:1000});
