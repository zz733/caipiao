...
draw2d.layout.connection.MazeConnectionRouter = draw2d.layout.connection.ConnectionRouter.extend({
	...
	simplifyDouglasPeucker: function(points, sqTolerance) {
		var len = points.length,
			...
			first = 0,
			last = len - 1,
			i, maxSqDist, sqDist, index, firstStack = [],
			lastStack = [],
			newPoints = [];
...
draw2d.policy.canvas.BoundingboxSelectionPolicy = draw2d.policy.canvas.SingleSelectionPolicy.extend({
	...
	onMouseDown: function(canvas, x, y) {
		...
		currentSelection.each($.proxy(function(i, figure) {
			...
			if(figure instanceof draw2d.shape.basic.Line) {} else {
				if(canDragStart === false) {
					this.unselect(canvas, figure);
				}
			}
...
draw2d.shape.basic.Oval = draw2d.VectorFigure.extend({
	...
	init: function(width, height,customColor) {
		this._super();
		if(customColor!=null)this.setBackgroundColor(customColor);
		else
		...
		if(d < 0) {} else {
			if(d > 0) {
				...
				if((t_a < 0 || 1 < t_a) && (t_b < 0 || 1 < t_b)) {
					if((t_a < 0 && t_b < 0) || (t_a > 1 && t_b > 1)) {} else {}
				} else {
					if(0 <= t_a && t_a <= 1) {
...
//		this.fontColor = new draw2d.util.Color("#080808");
		if(customFontSize)this.fontSize = customFontSize;
...
draw2d.shape.basic.Line = draw2d.Figure.extend({
	...
	init: function(startX, startY, endX, endY,customLineColor) {
		...
		if(customLineColor!=null)this.lineColor = customLineColor;
		else
