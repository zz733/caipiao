	...
	function build() {
		...
		svg.Font = new (function() {
			...
			this.CreateFont = function(fontStyle, fontVariant, fontWeight,
				...
				var f = inherit != null ? this.Parse(inherit) : this
						.CreateFont('', '', '', '', '', svg.ctx.font);
				return {
		...
		svg.ToNumberArray = function(s) {
			var a = svg.trim(svg.compressSpaces((s || '').replace(/,/g, ' ')))
					.split(' ');
		...
		svg.BoundingBox = function(x1, y1, x2, y2) { // pass in initial
			...
			this.addBezierCurve = function(p0x, p0y, p1x, p1y, p2x, p2y, p3x,
				...
				for (i = 0; i <= 1; i++) {
					var f = function(t) {
						return Math.pow(1 - t, 3) * p0[i] + 3
								* Math.pow(1 - t, 2) * t * p1[i] + 3 * (1 - t)
								* Math.pow(t, 2) * p2[i] + Math.pow(t, 3)
								* p3[i];
					}
		...
		svg.Transform = function(v) {
			...
			this.Type.scale = function(s) {
				...
				this.applyToPoint = function(p) {
					p.applyTransform([this.p.x || 0.0, 0, 0, this.p.y || 0.0,
							0, 0]);
				}
			}
		...
		svg.Element.ElementBase = function(node) {
			...
			this.render = function(ctx) {
				...
				if (this.attribute('mask').hasValue()) {
					var mask = this.attribute('mask').Definition
							.getDefinition();
					if (mask != null)
						mask.apply(ctx, this);
				} else if (this.style('filter').hasValue()) {
					var filter = this.style('filter').Definition
							.getDefinition();
					if (filter != null)
			...
			this.setContext = function(ctx) {
				...
				if (this.style('fill').Definition.isUrl()) {
					...
					if (this.style('fill-opacity').hasValue())
						fillStyle = fillStyle.Color.addOpacity(this
								.style('fill-opacity').value);
					ctx.fillStyle = (fillStyle.value == 'none'
							? 'rgba(0,0,0,0)'
							: fillStyle.value);
				}
				...
				if (this.style('stroke').Definition.isUrl()) {
					...
					if (this.style('stroke-opacity').hasValue())
						strokeStyle = strokeStyle.Color.addOpacity(this
				...
				if (this.style('stroke-width').hasValue())
					ctx.lineWidth = this.style('stroke-width').Length
							.toPixels();
				...
				if (this.attribute('transform').hasValue()) {
					var transform = new svg.Transform(this
				...
				if (this.attribute('clip-path').hasValue()) {
					var clip = this.attribute('clip-path').Definition
							.getDefinition();
					if (clip != null)
						clip.apply(ctx);
				}
			...
			this.renderChildren = function(ctx) {
				...
				if (markers != null) {
					if (this.style('marker-start').Definition.isUrl()) {
						var marker = this.style('marker-start').Definition
								.getDefinition();
						marker.render(ctx, markers[0][0], markers[0][1]);
					}
					if (this.style('marker-mid').Definition.isUrl()) {
						var marker = this.style('marker-mid').Definition
								.getDefinition();
					...
					if (this.style('marker-end').Definition.isUrl()) {
						var marker = this.style('marker-end').Definition
								.getDefinition();
			...
			this.setContext = function(ctx) {
				...
				if (this.attribute('viewBox').hasValue()) {
					...
					svg.AspectRatio(ctx,
							...
							height, minX, minY, this.attribute('refX').value,
		...
		svg.Element.rect = function(node) {
			...
			this.path = function(ctx) {
				...
				if (ctx != null) {
					...
					ctx.quadraticCurveTo(x + width, y + height, x + width - rx,
							y + height)
			...
			this.path = function(ctx) {
				...
				if (ctx != null) {
					...
					ctx.bezierCurveTo(cx + (KAPPA * rx), cy - ry, cx + rx, cy
									- (KAPPA * ry), cx + rx, cy);
					ctx.bezierCurveTo(cx + rx, cy + (KAPPA * ry), cx
									+ (KAPPA * rx), cy + ry, cx, cy + ry);
					ctx.bezierCurveTo(cx - (KAPPA * rx), cy + ry, cx - rx, cy
									+ (KAPPA * ry), cx - rx, cy);
					ctx.bezierCurveTo(cx - rx, cy - (KAPPA * ry), cx
									- (KAPPA * rx), cy - ry, cx, cy - ry);
		...
		svg.Element.line = function(node) {
			...
			this.getPoints = function() {
				return [
						new svg.Point(
								this.attribute('x1').Length.toPixels('x'), this
										.attribute('y1').Length.toPixels('y')),
						new svg.Point(
								this.attribute('x2').Length.toPixels('x'), this
		...
		svg.Element.path = function(node) {
			...
			this.PathParser = new (function(d) {
				...
				this.addMarker = function(p, from, priorTo) {
					...
					this.addMarkerAngle(p, from == null ? null : from
									.angleTo(p));
				}
					...
					switch (pp.command.toUpperCase()) {
						...
						case 'C' :
							while (!pp.isCommandOrEnd()) {
								...
								if (ctx != null)
									ctx.bezierCurveTo(p1.x, p1.y, cntrl.x,
											cntrl.y, cp.x, cp.y);
							}
							break;
						case 'S' :
						...
						case 'Q' :
							while (!pp.isCommandOrEnd()) {
								...
								bb.addQuadraticCurve(curr.x, curr.y, cntrl.x,
										cntrl.y, cp.x, cp.y);
								if (ctx != null)
									ctx.quadraticCurveTo(cntrl.x, cntrl.y,
											cp.x, cp.y);
							}
							break;
						case 'T' :
						...
						case 'A' :
							while (!pp.isCommandOrEnd()) {
								...
								// x1', y1'
								var currp = new svg.Point(Math
												.cos(xAxisRotation)
												* (curr.x - cp.x)
												/ 2.0
												+ Math.sin(xAxisRotation)
												* (curr.y - cp.y) / 2.0, -Math
												.sin(xAxisRotation)
												* (curr.x - cp.x)
												/ 2.0
								...
								// for markers
								var halfWay = new svg.Point(centp.x - rx
												* Math.cos((a1 + ad) / 2),
		...
		svg.Element.pattern = function(node) {
			...
			this.createPattern = function(ctx, element) {
				...
				tempSvg.attributes['x'] = new svg.Property('x', this
								.attribute('x').value);
				tempSvg.attributes['y'] = new svg.Property('y', this
								.attribute('y').value);
		...
		svg.Element.GradientBase = function(node) {
			...
			this.createGradient = function(ctx, element) {
				...
				if (this.attribute('xlink:href').hasValue()) {
					stopsContainer = this.attribute('xlink:href').Definition
							.getDefinition();
				}
		...
		svg.Element.radialGradient = function(node) {
			...
			this.getGradient = function(ctx, element) {
				...
				var r = (this.gradientUnits == 'objectBoundingBox' ? (bb
						.width() + bb.height())
						/ 2.0 * this.attribute('r').numValue() : this
						.attribute('r').Length.toPixels());
		...
		svg.Element.stop = function(node) {
			...
			if (this.style('stop-opacity').hasValue())
				stopColor = stopColor.Color.addOpacity(this
						.style('stop-opacity').value);
		...
		svg.Element.text = function(node) {
			...
			this.renderChildren = function(ctx) {
				var textAnchor = this.style('text-anchor')
				...
				for (var i = 0; i < this.children.length; i++) {
					...
					if (textAnchor != 'start'
							&& (i == 0 || child.attribute('x').hasValue())) { // new
																				// group?
		...
		svg.Element.TextElementBase = function(node) {
			...
			this.renderChildren = function(ctx) {
				var customFont = this.parent.style('font-family').Definition
						.getDefinition();
				if (customFont != null) {
					...
					var dx = svg
							.ToNumberArray(this.parent.attribute('dx').value);
		...
		svg.Element.tref = function(node) {
			...
			this.getText = function() {
				var element = this.attribute('xlink:href').Definition
						.getDefinition();
				if (element != null)
		...
		svg.Element.a = function(node) {
			...
			this.renderChildren = function(ctx) {
				if (this.hasText) {
					...
					var fontSize = new svg.Property('fontSize', svg.Font
									.Parse(svg.ctx.font).fontSize);
					svg.Mouse.checkBoundingBox(this, new svg.BoundingBox(
									this.x, this.y
											- fontSize.Length.toPixels('y'),
		...
		svg.Element.use = function(node) {
			...
			this.getDefinition = function() {
				...
				if (this.attribute('width').hasValue())
					element.attribute('width', true).value = this
							.attribute('width').value;
				if (this.attribute('height').hasValue())
					element.attribute('height', true).value = this
							.attribute('height').value;
				return element;
			}
		...
		svg.Element.filter = function(node) {
			...
			this.apply = function(ctx, element) {
				...
				// render on me
				ctx.drawImage(c, 0, 0, width + 2 * px, height + 2 * py, x - px,
						y - py, width + 2 * px, height + 2 * py);
		...
		svg.Element.feGaussianBlur = function(node) {
			...
			this.apply = function(ctx, x, y, width, height) {
				...
				blur(ctx, width, height, this.attribute('stdDeviation')
								.numValue());
			}
		}
		...
		svg.loadXmlDoc = function(ctx, dom) {
			...
			if (svg.opts['ignoreMouse'] != true) {
				ctx.canvas.onclick = function(e) {
					var p = mapXY(new svg.Point(e != null
									? e.clientX
									: event.clientX, e != null
									? e.clientY
									: event.clientY));
			...
			var draw = function() {
				...
				if (svg.opts['ignoreDimensions'] != true) {
					...
					if (e.style('width').hasValue()) {
						ctx.canvas.width = e.style('width').Length
								.toPixels('x');
						ctx.canvas.style.width = ctx.canvas.width + 'px';
					}
					if (e.style('height').hasValue()) {
						ctx.canvas.height = e.style('height').Length
								.toPixels('y');
