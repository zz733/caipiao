...
var draw2d = {
	...
	ui : {},
	isTouchDevice : ((navigator.platform.indexOf("iPhone") != -1)
			|| (navigator.platform.indexOf("iPod") != -1) || (navigator.platform
			.indexOf("iPad") != -1))
};
...
draw2d.util.Color = Class.extend({
	...
	getIdealTextColor : function() {
		var nThreshold = 105;
		var bgDelta = (this.red * 0.299) + (this.green * 0.587)
				+ (this.blue * 0.114);
		return (255 - bgDelta < nThreshold)
				? new draw2d.util.Color(0, 0, 0)
				: new draw2d.util.Color(255, 255, 255);
	},
	...
	fadeTo : function(color, pc) {
		...
		var g = Math
				.floor(this.green + (pc * (color.green - this.green)) + 0.5);
...
draw2d.geo.Point = Class.extend({
			...
			lerp : function(that, t) {
				return new draw2d.geo.Point(this.x + (that.x - this.x) * t,
...
draw2d.geo.Rectangle = draw2d.geo.Point.extend({
	...
	moveInside : function(rect) {
		...
		if (newRect.w < this.w) {
			newRect.x = Math.min(newRect.x + newRect.w, this.x + this.w)
					- newRect.w;
		} else {
			newRect.x = this.x;
		}
		if (newRect.h < this.h) {
			newRect.y = Math.min(newRect.y + newRect.h, this.y + this.h)
					- newRect.h;
		} else {
			newRect.y = this.y;
		}
	...
	intersects : function(rect) {
		x11 = rect.x, y11 = rect.y, x12 = rect.x + rect.w, y12 = rect.y
				+ rect.h, x21 = this.x, y21 = this.y, x22 = this.x + this.w, y22 = this.y
				+ this.h;
...
draw2d.command.CommandStack = Class.extend({
	...
	markSaveLocation : function() {
		this.undostack = [];
		this.redostack = [];
		this.notifyListeners(new draw2d.command.Command(),
				draw2d.command.CommandStack.POST_EXECUTE);
	},
		...
		if (command) {
			...
			this
					.notifyListeners(command,
							draw2d.command.CommandStack.POST_UNDO);
		}
	},
	redo : function() {
		...
		if (command) {
			...
			this
					.notifyListeners(command,
							draw2d.command.CommandStack.POST_REDO);
		}
	},
...
draw2d.command.CommandStack.POST_MASK = draw2d.command.CommandStack.POST_EXECUTE
		| draw2d.command.CommandStack.POST_UNDO
		| draw2d.command.CommandStack.POST_REDO;
draw2d.command.CommandStack.PRE_MASK = draw2d.command.CommandStack.PRE_EXECUTE
		| draw2d.command.CommandStack.PRE_UNDO
		| draw2d.command.CommandStack.PRE_REDO;
...
draw2d.command.CommandConnect = draw2d.command.Command.extend({
			...
			execute : function() {
				...
				if (this.connection === null) {
					var result = draw2d.Connection.createConnection(
							this.source, this.target, optionalCallback,
							this.dropTarget);
		...
		if (this.figure instanceof draw2d.shape.node.Node
			...
			for (var i = 0; i < ports.getSize(); i++) {
				...
				for (var c = 0, c_size = port.getConnections().getSize(); c < c_size; c++) {
					if (!this.connections
							.contains(port.getConnections().get(c))) {
...
draw2d.layout.connection.DirectRouter = draw2d.layout.connection.ConnectionRouter
		.extend({
...
draw2d.layout.connection.JunctionRouter = draw2d.layout.connection.ConnectionRouter
		.extend({
...
draw2d.layout.connection.ManhattanConnectionRouter = draw2d.layout.connection.ConnectionRouter
		.extend({
					...
					if ((xDiff > 0) && ((yDiff * yDiff) < this.TOL)
						...
						if (xDiff < 0) {
							...
							if (((yDiff > 0) && (toDir === DOWN))
								...
								if (fromDir == toDir) {
									var pos = Math.min(fromPt.x, toPt.x)
											- this.MINDIST;
					...
					if (fromDir === RIGHT) {
						if ((xDiff < 0) && ((yDiff * yDiff) < this.TOL)
							...
							if (xDiff > 0) {
								...
								if (((yDiff > 0) && (toDir === DOWN))
										|| ((yDiff < 0) && (toDir === UP))) {
									point = new draw2d.geo.Point(toPt.x,
											fromPt.y);
								} else {
									if (fromDir === toDir) {
										var pos = Math.max(fromPt.x, toPt.x)
												+ this.MINDIST;
										point = new draw2d.geo.Point(pos,
												fromPt.y);
									} else {
										point = new draw2d.geo.Point(fromPt.x
														- (xDiff / 2), fromPt.y);
...
draw2d.layout.connection.ManhattanBridgedConnectionRouter = draw2d.layout.connection.ManhattanConnectionRouter
		.extend({
...
draw2d.layout.connection.CircuitConnectionRouter = draw2d.layout.connection.ManhattanConnectionRouter
		.extend({
			...
			onUninstall : function(connection) {
				if (typeof connection.junctionNodes !== "undefined"
			...
			setBridgeRadius : function(radius) {
				...
				this.bridge_LR = [" r", 0.5, -0.5, radius - (radius / 2),
						-(radius - radius / 4), radius, -radius,
						radius + (radius / 2), -(radius - radius / 4),
						radius * 2, "0 "].join(" ");
				this.bridge_RL = [" r", -0.5, -0.5, -(radius - (radius / 2)),
						-(radius - radius / 4), -radius, -radius,
						-(radius + (radius / 2)), -(radius - radius / 4),
						-radius * 2, "0 "].join(" ");
			},
			route : function(conn, oldJunctionPoints) {
				...
				var intersectionForCalc = intersectionsASC;
				var i = 0;
				if (typeof conn.junctionNodes !== "undefined"
						&& conn.junctionNodes !== null) {
				...
				for (i = 1; i < ps.getSize(); i++) {
					...
					intersectionForCalc.each($.proxy(function(ii, interP) {
						if (draw2d.shape.basic.Line.hit(1, oldP.x, oldP.y, p.x,
								p.y, interP.x, interP.y) === true) {
							if (conn.sharingPorts(interP.other)) {
								var other = interP.other;
								var otherZ = other.getZOrder();
								var connZ = conn.getZOrder();
								if (connZ < otherZ) {
									var junctionNode = conn.canvas.paper
											.ellipse(interP.x, interP.y,
													this.junctionRadius,
													this.junctionRadius).attr({
														fill : conn.lineColor
																.hash()
													});
									...
									if (this.abortRoutingOnFirstJunctionNode === true) {
										if (conn.getSource() == other
												.getSource()
												|| conn.getSource() == other
														.getTarget()) {
											path = ["M", (interP.x | 0) + 0.5,
													" ", (interP.y | 0) + 0.5];
											if (lastJunctionNode !== null) {
												lastJunctionNode.remove();
												conn.junctionNodes
														.exclude(lastJunctionNode);
											}
										}
...
draw2d.layout.connection.SplineConnectionRouter = draw2d.layout.connection.ManhattanConnectionRouter
		.extend({
					...
					route : function(conn, oldJunctionPoints) {
						var i = 0;
						var fromPt = conn.getStartPoint();
						var fromDir = conn.getSource().getConnectionDirection(
								conn, conn.getTarget());
						var toPt = conn.getEndPoint();
						var toDir = conn.getTarget().getConnectionDirection(
								conn, conn.getSource());
						this._route(conn, toPt, toDir, fromPt, fromDir);
						var ps = conn.getPoints();
						conn.oldPoint = null;
...
draw2d.layout.connection.FanConnectionRouter = draw2d.layout.connection.DirectRouter
		.extend({
			...
			route : function(conn, oldJunctionPoints) {
				...
				for (var i = 0; i < lines.getSize(); i++) {
					...
					if (figure.getTarget() === conn.getTarget()
							|| figure.getSource() === conn.getTarget()) {
			...
			routeCollision : function(conn, index) {
				...
				var separation = 15;
				var midPoint = new draw2d.geo.Point((end.x + start.x) / 2,
						(end.y + start.y) / 2);
				var position = end.getPosition(start);
				var ray;
				if (position == draw2d.geo.PositionConstants.SOUTH
						|| position == draw2d.geo.PositionConstants.EAST) {
				...
				var bendPoint;
				if (index % 2 === 0) {
					bendPoint = new draw2d.geo.Point(midPoint.x + (index / 2)
									* (-1 * ySeparation), midPoint.y
									+ (index / 2) * xSeparation);
				} else {
					bendPoint = new draw2d.geo.Point(midPoint.x + (index / 2)
									* ySeparation, midPoint.y + (index / 2)
									* (-1 * xSeparation));
				}
				conn.addPoint(start);
...
draw2d.layout.connection.MazeConnectionRouter = draw2d.layout.connection.ConnectionRouter
		.extend({
			...
			_route : function(conn, fromPt, fromDir, toPt, toDir) {
				...
				oldToPt = toPt;
				oldFromPt = fromPt;
				fromPt = this.getAddjustedPoint(fromPt, fromDir,
						this.portOutletOffset);
				toPt = this.getAddjustedPoint(toPt, toDir,
						this.portOutletOffset);
				var grid = this.generateNoGoGrid(conn, fromPt, fromDir, toPt,
						toDir);
				var path = this.finder.findPath(fromPt.x >> shift,
						fromPt.y >> shift, toPt.x >> shift, toPt.y >> shift,
						grid);
				...
				if (this.useDebug) {
					...
					for (var i = 0; i < grid.width; i++) {
						for (var j = 0; j < grid.height; j++) {
							if (!grid.isWalkableAt(i, j)) {
								ROUTER_RECTS.push(conn.canvas.paper.rect(
										i << shift, j << shift, 1 << shift,
										1 << shift).attr({
											"fill" : "red",
											"opacity" : "0.1"
										}));
					...
					ROUTER_RECTS.push(conn.canvas.paper.rect(fromPt.x - 3,
							fromPt.y - 3, 6, 6).attr({
								"fill" : "#ff0000",
								"opacity" : "0.8"
							}));
					ROUTER_RECTS.push(conn.canvas.paper.rect(toPt.x - 3,
							toPt.y - 3, 6, 6).attr({
								"fill" : "#ff0000",
								"opacity" : "0.8"
							}));
					...
					ROUTER_RECTS.push(conn.canvas.paper.path(svgPathBefore)
							.attr({
										"stroke" : "#0000ff"
									}));
				}
			...
			generateNoGoGrid : function(conn, fromPt, fromDir, toPt, toDir) {
				...
				figures.each(function(i, e) {
							...
							if (box.hitTest(fromPt.x, fromPt.y) === true
									|| box.hitTest(toPt.x, toPt.y)) {
								return;
							}
							var x = box.x >> shift;
							var y = box.y >> shift;
			...
			simplifyDouglasPeucker : function(points, sqTolerance) {
				var len = points.length, MarkerArray = (typeof Uint8Array !== undefined
						+ "") ? Uint8Array : Array, markers = new MarkerArray(len), first = 0, last = len
						- 1, i, maxSqDist, sqDist, index, firstStack = [], lastStack = [], newPoints = [];
				markers[first] = markers[last] = 1;
				while (last) {
					maxSqDist = 0;
					for (i = first + 1; i < last; i++) {
						sqDist = this.getSquareSegmentDistance(points[i],
			...
			simplify : function(points, tolerance, highestQuality) {
				var sqTolerance = (tolerance !== undefined) ? tolerance
						* tolerance : 1;
				if (!highestQuality) {
...
draw2d.layout.connection.SketchConnectionRouter = draw2d.layout.connection.MazeConnectionRouter
		.extend({
			...
			add : function(canvas, figureToAdd) {
				...
				for (i = 0; i < figures.getSize(); i++) {
					...
					if (figure !== figureToAdd) {
						...
						if (dis < this.MIN_MARGIN) {
							oct = this.determineOctant(boundingBox, figure
											.getBoundingBox());
							switch (oct) {
								case 2 :
									changes[2].x = Math.max(changes[2].x,
											this.MIN_MARGIN - dis);
									changes[3].x = Math.max(changes[3].x,
											this.MIN_MARGIN - dis);
									changes[4].x = Math.max(changes[4].x,
											this.MIN_MARGIN - dis);
									break;
								case 3 :
									changes[2].x = Math.max(changes[2].x,
								...
								case 4 :
									...
									changes[4].x = Math.max(changes[4].x,
											this.MIN_MARGIN - dis);
									changes[4].y = Math.max(changes[4].y,
											this.MIN_MARGIN - dis);
									changes[5].y = Math.max(changes[5].y,
											this.MIN_MARGIN - dis);
									changes[6].y = Math.max(changes[6].y,
											this.MIN_MARGIN - dis);
									break;
								case 5 :
									changes[4].y = Math.max(changes[4].y,
								...
								case 6 :
									...
									break;
								case 8 :
									dis = (boundingBox.getBottomRight()
											.getDistance(figure
													.getBoundingBox()
													.getTopLeft()))
											| 0;
									...
									changes[6].y = Math.max(changes[6].y,
											this.MIN_MARGIN + dis);
									changes[8].x = Math.max(changes[8].x,
											this.MIN_MARGIN + dis);
							}
						}
					}
				...
				for (i = 0; i < figures.getSize(); i++) {
					...
					if (figure !== figureToAdd) {
						...
						if (currentOctChanges.x !== 0
								|| currentOctChanges.y !== 0) {
							result
									.add(new draw2d.layout.mesh.ProposedMeshChange(
											figure, currentOctChanges.x,
											currentOctChanges.y));
						}
					}
				}
...
draw2d.layout.locator.InputPortLocator = draw2d.layout.locator.PortLocator
		.extend({
					...
					relocate : function(index, figure) {
						...
						this
								.applyConsiderRotation(figure, 0, gap
												* (index + 1));
					}
				});
draw2d.layout.locator.OutputPortLocator = draw2d.layout.locator.PortLocator
		.extend({
					...
					relocate : function(index, figure) {
						var node = figure.getParent();
						var gap = node.getHeight()
								/ (node.getOutputPorts().getSize() + 1);
						this.applyConsiderRotation(figure, node.getWidth(), gap
										* (index + 1));
					}
				});
...
draw2d.layout.locator.ManhattanMidpointLocator = draw2d.layout.locator.ConnectionLocator
		.extend({
...
draw2d.layout.locator.PolylineMidpointLocator = draw2d.layout.locator.ManhattanMidpointLocator
		.extend({
					...
					relocate : function(index, target) {
						...
						if (points.getSize() % 2 === 0) {
							...
							target.setPosition(p1.x - (target.getWidth() / 2),
									p1.y - (target.getHeight() / 2));
						}
					}
				});
draw2d.layout.locator.TopLocator = draw2d.layout.locator.Locator.extend({
			...
			relocate : function(index, target) {
				...
				if (target instanceof draw2d.Port) {
					...
					target.setPosition(boundingBox.w / 2
									- (targetBoundingBox.w / 2),
							-(targetBoundingBox.h + 2));
				}
			}
		});
draw2d.layout.locator.BottomLocator = draw2d.layout.locator.Locator.extend({
			...
			relocate : function(index, target) {
				...
				if (target instanceof draw2d.Port) {
					...
					target.setPosition(boundingBox.w / 2 - targetBoundingBox.w
									/ 2, 2 + boundingBox.h);
				}
			}
		});
draw2d.layout.locator.LeftLocator = draw2d.layout.locator.Locator.extend({
			...
			relocate : function(index, target) {
				...
				var boundingBox = parent.getBoundingBox();
				var targetBoundingBox = target.getBoundingBox();
				target.setPosition(-targetBoundingBox.w - 5,
						(boundingBox.h / 2) - (targetBoundingBox.h / 2));
			}
		});
draw2d.layout.locator.RightLocator = draw2d.layout.locator.Locator.extend({
			...
			relocate : function(index, target) {
				...
				var boundingBox = parent.getBoundingBox();
				var targetBoundingBox = target.getBoundingBox();
				target.setPosition(boundingBox.w + 5, (boundingBox.h / 2)
								- (targetBoundingBox.h / 2));
			}
		});
...
draw2d.policy.canvas.SelectionPolicy = draw2d.policy.canvas.CanvasPolicy
		.extend({
...
draw2d.policy.canvas.SingleSelectionPolicy = draw2d.policy.canvas.SelectionPolicy
		.extend({
			...
			onMouseDown : function(canvas, x, y) {
				...
				if (figure !== canvas.getSelection().getPrimary()
						&& figure !== null && figure.isSelectable() === true) {
					this.select(canvas, figure);
					if (figure instanceof draw2d.shape.basic.Line) {
						if (!(figure instanceof draw2d.Connection)) {
							canvas.draggingLineCommand = figure
			...
			onMouseDrag : function(canvas, dx, dy, dx2, dy2) {
				...
				if (this.mouseDraggingElement !== null) {
					if (canvas.linesToRepaintAfterDragDrop.isEmpty() === true
							&& (this.mouseDraggingElement instanceof draw2d.shape.node.Node)) {
						var nodeConnections = this.mouseDraggingElement
								.getConnections();
						var newLineIntersections = canvas.lineIntersections
								.clone();
						canvas.lineIntersections.each($.proxy(
								function(i, inter) {
									if (nodeConnections.contains(inter.line)
											|| nodeConnections
													.contains(inter.other)) {
										newLineIntersections.remove(inter);
										canvas.linesToRepaintAfterDragDrop
												.add(inter.line);
										canvas.linesToRepaintAfterDragDrop
												.add(inter.other);
									}
						...
						canvas.linesToRepaintAfterDragDrop.each(function(i,
										line) {
					...
					var target = canvas.getBestFigure(p.x, p.y,
							this.mouseDraggingElement);
					if (target !== canvas.currentDropTarget) {
						if (canvas.currentDropTarget !== null) {
							canvas.currentDropTarget
									.onDragLeave(this.mouseDraggingElement);
							canvas.currentDropTarget = null;
						}
						if (target !== null) {
							canvas.currentDropTarget = target
									.onDragEnter(this.mouseDraggingElement);
						}
					}
				} else {
					if (this.mouseDownElement !== null
							&& !(this.mouseDownElement instanceof draw2d.Connection)) {
			...
			onMouseUp : function(canvas, x, y) {
				if (this.mouseDraggingElement !== null) {
					...
					if (!sel.contains(this.mouseDraggingElement)) {
						...
						canvas.getSelection().getAll().each(
								function(i, figure) {
									figure.onDragEnd();
								});
					...
					if (canvas.currentDropTarget !== null) {
						this.mouseDraggingElement
								.onDrop(canvas.currentDropTarget);
						canvas.currentDropTarget
								.onDragLeave(this.mouseDraggingElement);
						canvas.currentDropTarget = null;
					}
...
draw2d.policy.canvas.PanningSelectionPolicy = draw2d.policy.canvas.SingleSelectionPolicy
		.extend({
...
draw2d.policy.canvas.BoundingboxSelectionPolicy = draw2d.policy.canvas.SingleSelectionPolicy
		.extend({
					...
					onMouseDown : function(canvas, x, y) {
						...
						if (this.mouseDownElement !== null
								...
								&& !currentSelection
										.contains(this.mouseDownElement)) {
						...
						currentSelection.each($.proxy(function(i, figure) {
									var canDragStart = figure.onDragStart(
											figure.getAbsoluteX(), figure
													.getAbsoluteY());
									if (figure instanceof draw2d.shape.basic.Line) {
									} else {
										if (canDragStart === false) {
					...
					onMouseDrag : function(canvas, dx, dy, dx2, dy2) {
						...
						if (this.mouseDraggingElement === null
							...
							this.boundingBoxFigure2
									.setColor(new draw2d.util.Color(84, 151,
											220));
							this.boundingBoxFigure2.setBackgroundColor(null);
						}
						if (this.boundingBoxFigure1 !== null) {
							this.boundingBoxFigure1.setDimension(Math.abs(dx),
									Math.abs(dy));
							this.boundingBoxFigure1.setPosition(this.x
											+ Math.min(0, dx), this.y
											+ Math.min(0, dy));
							this.boundingBoxFigure2.setDimension(Math.abs(dx),
									Math.abs(dy));
							this.boundingBoxFigure2.setPosition(this.x
											+ Math.min(0, dx), this.y
											+ Math.min(0, dy));
						}
					},
					onMouseUp : function(canvas, x, y) {
						if (this.mouseDownElement === null) {
							canvas.getSelection().getAll().each($.proxy(
									function(i, figure) {
										this.unselect(canvas, figure);
									}, this));
						} else {
							if (this.mouseDownElement !== null
								...
								if (!sel.contains(this.mouseDownElement)) {
									canvas.getSelection().getAll().each($
											.proxy(function(i, figure) {
														this.unselect(canvas,
																figure);
													}, this));
								}
							}
						...
						this._super(canvas, x, y);
						if (this.boundingBoxFigure1 !== null) {
							var selectionRect = this.boundingBoxFigure1
									.getBoundingBox();
							canvas.getFigures().each($.proxy(
									function(i, figure) {
										if (figure.getBoundingBox()
												.isInside(selectionRect)) {
											var canDragStart = figure
													.onDragStart(
															figure
																	.getAbsoluteX(),
															figure
...
draw2d.policy.canvas.ReadOnlySelectionPolicy = draw2d.policy.canvas.SelectionPolicy
		.extend({
...
draw2d.policy.canvas.DecorationPolicy = draw2d.policy.canvas.CanvasPolicy
		.extend({
...
draw2d.policy.canvas.FadeoutDecorationPolicy = draw2d.policy.canvas.DecorationPolicy
		.extend({
					...
					onInstall : function(canvas) {
						this.canvas = canvas;
						this.timerId = window.setInterval($.proxy(this.onTimer,
										this), 100);
					...
					onUninstall : function(canvas) {
						...
						this.canvas.getAllPorts().each($.proxy(
								function(i, port) {
									port.setAlpha(1);
								}, this));
					},
					onTimer : function() {
						...
						if (this.hidePortsCounter <= 0 && this.alpha > 0) {
							...
							this.canvas.getAllPorts().each($.proxy(function(i,
											port) {
										port.setAlpha(this.alpha);
									}, this));
							this.canvas.getSelection().getAll().each($.proxy(
									function(i, figure) {
										figure.selectionHandles.each($.proxy(
												function(i, handle) {
							...
							if (this.hidePortsCounter > 0 && this.alpha !== 1) {
								...
								this.canvas.getSelection().getAll().each($
										.proxy(function(i, figure) {
											figure.selectionHandles.each($
													.proxy(function(i, handle) {
														handle
																.setAlpha(this.alpha);
													}, this));
										}, this));
...
draw2d.policy.canvas.CoronaDecorationPolicy = draw2d.policy.canvas.DecorationPolicy
		.extend({
			...
			onMouseDrag : function(canvas, dx, dy, dx2, dy2) {
				this.updatePorts(canvas, this.startDragX + dx, this.startDragY
								+ dy);
			},
			updatePorts : function(canvas, x, y) {
				...
				figures.each(function(i, figure) {
							if (figure.isVisible() === true
									&& figure.hitTest(x, y, 50) === true
									&& figure instanceof draw2d.shape.node.Node) {
								figure.getPorts().each(function(i, p) {
											p.setVisible(true);
										});
							} else {
...
draw2d.policy.canvas.SnapToEditPolicy = draw2d.policy.canvas.CanvasPolicy
		.extend({
...
draw2d.policy.canvas.SnapToGridEditPolicy = draw2d.policy.canvas.SnapToEditPolicy
		.extend({
					...
					snap : function(canvas, figure, pos) {
						...
						pos.x = this.grid
								* Math
										.floor(((pos.x + this.grid / 2) / this.grid));
						pos.y = this.grid
								* Math
										.floor(((pos.y + this.grid / 2) / this.grid));
						pos.x = pos.x - snapPoint.x;
						pos.y = pos.y - snapPoint.y;
						return pos;
					},
					showGrid : function() {
						...
						for (var x = this.grid; x < w; x += this.grid) {
							this.lines.push(this.canvas.paper.path("M " + x
									+ " 0 l 0 " + h));
						}
						for (var y = this.grid; y < h; y += this.grid) {
							this.lines.push(this.canvas.paper.path("M 0 " + y
									+ " l " + w + " 0"));
						}
						this.lines.attr({
...
draw2d.policy.canvas.ShowGridEditPolicy = draw2d.policy.canvas.SnapToEditPolicy
		.extend({
...
draw2d.policy.canvas.SnapToGeometryEditPolicy = draw2d.policy.canvas.SnapToEditPolicy
		.extend({
			...
			snap : function(canvas, figure, pos) {
				if (figure instanceof draw2d.ResizeHandle) {
					...
					if ((snapDirections & draw2d.SnapToHelper.EAST_WEST)
							&& !(direction & draw2d.SnapToHelper.EAST_WEST)) {
						this.showVerticalLine(result.x);
					} else {
						this.hideVerticalLine();
					}
					if ((snapDirections & draw2d.SnapToHelper.NORTH_SOUTH)
							&& !(direction & draw2d.SnapToHelper.NORTH_SOUTH)) {
					...
					var inputBounds = new draw2d.geo.Rectangle(pos.x, pos.y,
							figure.getWidth(), figure.getHeight());
					var result = new draw2d.geo.Rectangle(pos.x, pos.y, figure
									.getWidth(), figure.getHeight());
					var snapDirections = draw2d.SnapToHelper.NSEW;
					var direction = this.snapRectangle(inputBounds, result);
					if ((snapDirections & draw2d.SnapToHelper.WEST)
							&& !(direction & draw2d.SnapToHelper.WEST)) {
						this.showVerticalLine(result.x);
					} else {
						if ((snapDirections & draw2d.SnapToHelper.EAST)
								&& !(direction & draw2d.SnapToHelper.EAST)) {
							this.showVerticalLine(result.getX()
									+ result.getWidth());
						} else {
							this.hideVerticalLine();
						}
					}
					if ((snapDirections & draw2d.SnapToHelper.NORTH)
							&& !(direction & draw2d.SnapToHelper.NORTH)) {
						this.showHorizontalLine(result.y);
					} else {
						if ((snapDirections & draw2d.SnapToHelper.SOUTH)
								&& !(direction & draw2d.SnapToHelper.SOUTH)) {
							this.showHorizontalLine(result.getY()
									+ result.getHeight());
						} else {
							this.hideHorizontalLine();
						}
			...
			snapRectangle : function(inputBounds, resultBounds) {
				...
				var snapDirectionsTopLeft = this.snapPoint(
						draw2d.SnapToHelper.NORTH_WEST, inputBounds
								.getTopLeft(), topLeftResult);
				resultBounds.x = topLeftResult.x;
				resultBounds.y = topLeftResult.y;
				var snapDirectionsBottomRight = this.snapPoint(
						draw2d.SnapToHelper.SOUTH_EAST, inputBounds
								.getBottomRight(), bottomRightResult);
				if (snapDirectionsTopLeft & draw2d.SnapToHelper.WEST) {
					resultBounds.x = bottomRightResult.x
							- inputBounds.getWidth();
				}
				if (snapDirectionsTopLeft & draw2d.SnapToHelper.NORTH) {
					resultBounds.y = bottomRightResult.y
							- inputBounds.getHeight();
				}
			...
			snapPoint : function(snapOrientation, inputPoint, resultPoint) {
				...
				if ((snapOrientation & draw2d.SnapToHelper.EAST) !== 0) {
					var rightCorrection = this.getCorrectionFor(this.cols,
							inputPoint.getX() - 1, 1);
				...
				if ((snapOrientation & draw2d.SnapToHelper.WEST) !== 0) {
					var leftCorrection = this.getCorrectionFor(this.cols,
							inputPoint.getX(), -1);
				...
				if ((snapOrientation & draw2d.SnapToHelper.SOUTH) !== 0) {
					var bottomCorrection = this.getCorrectionFor(this.rows,
							inputPoint.getY() - 1, 1);
				...
				if ((snapOrientation & draw2d.SnapToHelper.NORTH) !== 0) {
					var topCorrection = this.getCorrectionFor(this.rows,
							inputPoint.getY(), -1);
			...
			showVerticalLine : function(x) {
				...
				this.vline = this.canvas.paper.path("M " + x + " 0 l 0 "
						+ this.canvas.getHeight()).attr({
			...
			showHorizontalLine : function(y) {
				...
				this.hline = this.canvas.paper.path("M 0 " + y + " l "
						+ this.canvas.getWidth() + " 0").attr({
							"stroke" : this.LINE_COLOR,
							"stroke-width" : 1
						});
			},
...
draw2d.policy.figure.RegionEditPolicy = draw2d.policy.figure.DragDropEditPolicy
		.extend({
			...
			adjustPosition : function(figure, x, y) {
				var r = null;
				if (x instanceof draw2d.geo.Point) {
					r = new draw2d.geo.Rectangle(x.x, x.y, figure.getWidth(),
							figure.getHeight());
				} else {
					r = new draw2d.geo.Rectangle(x, y, figure.getWidth(),
							figure.getHeight());
				}
...
draw2d.policy.figure.HorizontalEditPolicy = draw2d.policy.figure.DragDropEditPolicy
		.extend({
...
draw2d.policy.figure.VerticalEditPolicy = draw2d.policy.figure.DragDropEditPolicy
		.extend({
...
draw2d.policy.figure.SelectionFeedbackPolicy = draw2d.policy.figure.DragDropEditPolicy
		.extend({
...
draw2d.policy.figure.RectangleSelectionFeedbackPolicy = draw2d.policy.figure.SelectionFeedbackPolicy
		.extend({
			...
			moved : function(canvas, figure) {
				...
				if (figure.isStrechable()) {
					...
					r2.setPosition(xPos + (objWidth / 2) - (r2.getWidth() / 2),
							yPos - r2.getHeight());
					r4.setPosition(xPos + objWidth, yPos + (objHeight / 2)
									- (r4.getHeight() / 2));
					r6.setPosition(xPos + (objWidth / 2) - (r6.getWidth() / 2),
							yPos + objHeight);
					r8.setPosition(xPos - r8.getWidth(), yPos + (objHeight / 2)
									- (r8.getHeight() / 2));
				}
...
draw2d.policy.figure.BigRectangleSelectionFeedbackPolicy = draw2d.policy.figure.RectangleSelectionFeedbackPolicy
		.extend({
...
draw2d.policy.figure.RoundRectangleSelectionFeedbackPolicy = draw2d.policy.figure.RectangleSelectionFeedbackPolicy
		.extend({
...
draw2d.policy.figure.BusSelectionFeedbackPolicy = draw2d.policy.figure.SelectionFeedbackPolicy
		.extend({
					...
					moved : function(canvas, figure) {
						...
						var xPos = figure.getX();
						var yPos = figure.getY();
						r2.setPosition(xPos + (objWidth / 2)
										- (r2.getWidth() / 2), yPos
										- r2.getHeight());
						r4.setPosition(xPos + objWidth, yPos + (objHeight / 2)
										- (r4.getHeight() / 2));
						r6
								.setPosition(xPos + (objWidth / 2)
												- (r6.getWidth() / 2), yPos
												+ objHeight);
...
draw2d.policy.figure.VBusSelectionFeedbackPolicy = draw2d.policy.figure.BusSelectionFeedbackPolicy
		.extend({
...
draw2d.policy.figure.HBusSelectionFeedbackPolicy = draw2d.policy.figure.BusSelectionFeedbackPolicy
		.extend({
...
draw2d.policy.figure.AntSelectionFeedbackPolicy = draw2d.policy.figure.SelectionFeedbackPolicy
		.extend({
...
draw2d.policy.figure.GlowSelectionFeedbackPolicy = draw2d.policy.figure.SelectionFeedbackPolicy
		.extend({
...
draw2d.policy.figure.SlimSelectionFeedbackPolicy = draw2d.policy.figure.RectangleSelectionFeedbackPolicy
		.extend({
...
draw2d.policy.line.LineSelectionFeedbackPolicy = draw2d.policy.figure.SelectionFeedbackPolicy
		.extend({
			...
			onSelect : function(canvas, figure, isPrimarySelection) {
				if (figure.selectionHandles.isEmpty()) {
					figure.selectionHandles
							.add(new draw2d.shape.basic.LineStartResizeHandle(figure));
					figure.selectionHandles
							.add(new draw2d.shape.basic.LineEndResizeHandle(figure));
					figure.selectionHandles.each(function(i, e) {
			...
			moved : function(canvas, figure) {
				...
				resizeHandleStart.setPosition(startPoint.x - resizeWidthHalf,
						startPoint.y - resizeHeightHalf);
				resizeHandleEnd.setPosition(endPoint.x - resizeWidthHalf,
						endPoint.y - resizeHeightHalf);
			}
		});
draw2d.policy.line.JunctionSelectionFeedbackPolicy = draw2d.policy.line.LineSelectionFeedbackPolicy
		.extend({
					...
					onSelect : function(canvas, connection, isPrimarySelection) {
						...
						for (var i = 1; i < (points.getSize() - 1); i++) {
							var handle = new draw2d.shape.basic.JunctionResizeHandle(
									connection, i);
					...
					moved : function(canvas, figure) {
						...
						for (var i = 2; i < figure.selectionHandles.getSize(); i++) {
							...
							handle.setPosition(point.x - resizeWidthHalf,
									point.y - resizeHeightHalf);
						}
					}
				});
draw2d.policy.port.PortFeedbackPolicy = draw2d.policy.figure.DragDropEditPolicy
		.extend({
...
draw2d.policy.port.ElasticStrapFeedbackPolicy = draw2d.policy.port.PortFeedbackPolicy
		.extend({
					...
					onDrag : function(canvas, figure) {
						...
						this.connectionLine.setEndPoint(figure.getAbsoluteX(),
								figure.getAbsoluteY());
					},
					onDragEnd : function(canvas, figure) {
...
draw2d.policy.port.IntrusivePortsFeedbackPolicy = draw2d.policy.port.PortFeedbackPolicy
		.extend({
			...
			onDrag : function(canvas, figure) {
				...
				var y1 = figure.oy + figure.getParent().getAbsoluteY();
				this.connectionLine.setStartPoint(x1, y1);
				this.connectionLine.setEndPoint(figure.getAbsoluteX(), figure
								.getAbsoluteY());
			},
			onDragEnd : function(canvas, figure) {
...
draw2d.Canvas = Class.extend({
	NAME : "draw2d.Canvas",
	init : function(canvasId) { // here is init svg---create by linyi
		...
		this.html.droppable({
					...
					drop : $.proxy(function(event, ui) {
								...
								var pos = this.fromDocumentToCanvasCoordinate(
										event.clientX, event.clientY);
								this.onDrop(ui.draggable, pos.getX(), pos
												.getY());
							}, this)
				});
		...
		this
				.installEditPolicy(new draw2d.policy.canvas.BoundingboxSelectionPolicy());
		this.commandStack.addEventListener($.proxy(function(event) {
					if (event.isPostChangeEvent() === true) {
						...
						this.linesToRepaintAfterDragDrop.each(
								function(i, line) {
									line.svgPathString = null;
									line.repaint();
								});
		...
		this.html.bind("mousemove touchmove", $.proxy(function(event) {
							...
							if (this.mouseDown === false) {
								...
								try {
									var hover = this
											.getBestFigure(pos.x, pos.y);
									if (hover !== this.currentHoverFigure
											&& this.currentHoverFigure !== null) {
								...
								this.editPolicy.each($.proxy(
										function(i, policy) {
											policy.onMouseMove(this, pos.x,
													pos.y);
										}, this));
							} else {
								var diffXAbs = (event.clientX - this.mouseDownX)
										* this.zoomFactor;
								var diffYAbs = (event.clientY - this.mouseDownY)
										* this.zoomFactor;
								this.editPolicy.each($.proxy(
										function(i, policy) {
											policy.onMouseDrag(this, diffXAbs,
													diffYAbs,
													this.mouseDragDiffX
															- diffXAbs,
													this.mouseDragDiffY
							...
							switch (event.which) {
								...
								case 0 :
									...
									pos = this.fromDocumentToCanvasCoordinate(
											event.clientX, event.clientY);
									this.mouseDown = true;
									this.editPolicy.each($.proxy(function(i,
													policy) {
												policy.onMouseDown(this, pos.x,
														pos.y);
											}, this));
									break;
								case 3 :
		...
		$(document).bind("click", $.proxy(function(event) {
					event = this._getEvent(event);
					if (this.mouseDownX === event.clientX
							|| this.mouseDownY === event.clientY) {
						var pos = this.fromDocumentToCanvasCoordinate(
								event.clientX, event.clientY);
	...
	setZoom : function(zoomFactor, animated) {
		var _zoom = $.proxy(function(z) {
					...
					var viewBoxWidth = (this.initialWidth * this.zoomFactor)
							| 0;
					var viewBoxHeight = (this.initialHeight * this.zoomFactor)
							| 0;
	...
	fromDocumentToCanvasCoordinate : function(x, y) {
		return new draw2d.geo.Point((x - this.getAbsoluteX() + this
						.getScrollLeft())
						* this.zoomFactor, (y - this.getAbsoluteY() + this
						.getScrollTop())
						* this.zoomFactor);
	},
	_fromDocumentToCanvasCoordinate_IE8_HACK : function(x, y) {
		return new draw2d.geo.Point(
				(x - this.getAbsoluteX()) * this.zoomFactor, (y - this
						.getAbsoluteY())
						* this.zoomFactor);
	},
	...
	// 自定义方法
	getLinesLength : function() {
		return this.lines.getSize();
	},
	getFiguresLength : function() {
		return this.figures.getSize();
	},
	//
	...
	removeSelectionListener : function(w) {
		this.selectionListeners = this.selectionListeners.grep(function(
						listener) {
		...
		for (i = (this.figures.getSize() - 1); i >= 0; i--) {
			...
			var checkRecursive = function(children) {
				children.each(function(i, e) {
							...
							if (result === null && e.isVisible() === true
									&& e.hitTest(x, y) === true) {
								result = e;
							}
							return result === null;
	...
	getBestLine : function(x, y, lineToIgnore) {
		...
		for (var i = 0; i < count; i++) {
			var line = this.lines.get(i);
			if (line.isVisible() === true && line.hitTest(x, y) === true
					&& line !== lineToIgnore) {
				if (result === null) {
					result = line;
					break;
		...
		if (keyCode == 46 && this.selection.getPrimary() !== null) {
			this.commandStack
					.execute(this.selection
							.getPrimary()
							.createCommand(new draw2d.command.CommandType(draw2d.command.CommandType.DELETE)));
		} else {
			if (keyCode == 90 && ctrl) {
...
draw2d.Figure = Class.extend({
	...
	init : function(width, height) {
		...
		this
				.installEditPolicy(new draw2d.policy.figure.RectangleSelectionFeedbackPolicy());
	},
	...
	hasCssClass : function(className) {
		...
		return new RegExp(" " + $.trim(className) + " ").test(" "
				+ this.cssClass + " ");
	},
	...
	startTimer : function(milliSeconds) {
		...
		if (this.canvas !== null) {
			this.timerId = window.setInterval($.proxy(this.onTimer, this),
					this.timerInterval);
		}
		return this;
	},
	...
	createShapeElement : function() {
		throw "Inherited class [" + this.NAME
				+ "] must override the abstract method createShapeElement";
	},
	...
	getAbsoluteBounds : function() {
		return new draw2d.geo.Rectangle(this.getAbsoluteX(), this
						.getAbsoluteY(), this.getWidth(), this.getHeight());
	},
	setPosition : function(x, y) {
		if (x instanceof draw2d.geo.Point) {
			this.x = x.x;
...
draw2d.shape.node.Node = draw2d.Figure.extend({
	...
	addPort : function(port, locator) {
		...
		if ((typeof locator !== "undefined")
				&& (locator instanceof draw2d.layout.locator.Locator)) {
...
draw2d.shape.basic.Rectangle = draw2d.VectorFigure.extend({
			...
			applyTransformation : function() {
				...
				if (this.getRotationAngle() === 90
						|| this.getRotationAngle() === 270) {
					var ratio = this.getHeight() / this.getWidth();
					var rs = "...S" + ratio + "," + 1 / ratio + ","
...
draw2d.SetFigure = draw2d.shape.basic.Rectangle.extend({
			...
			applyTransformation : function() {
				var s = "S" + this.scaleX + "," + this.scaleY + ",0,0 " + "R"
						+ this.rotationAngle + ","
						+ ((this.getWidth() / 2) | 0) + ","
						+ ((this.getHeight() / 2) | 0) + "T"
			...
			createShapeElement : function() {
				var shape = this.canvas.paper.rect(this.getX(), this.getY(),
		...
		try {
			...
			svgDOM.children().each(function(i, element) {
				...
				if (typeof attr["stroke-width"] === "undefined") {
					attr["stroke-width"] = (typeof attr.stroke === "undefined"
							? 0
							: 1.2);
				}
				switch (node) {
					...
					case "path" :
						...
						break;
					case "line" :
						attr.d = "M " + attr.x1 + " " + attr.y1 + "L" + attr.x2
								+ " " + attr.y2;
						attr.fill = "none";
						shape = canvas.paper.path(attr.d);
						break;
					...
					case "text" :
						if (element.childNodes.length > 0) {
							...
							do {
								...
								var subShape = canvas.paper.text(0, 0, $(child)
												.text());
								var subAttr = {
...
draw2d.shape.node.Hub = draw2d.shape.basic.Rectangle.extend({
	...
	init : function(width, height, label) {
		this.label = null;
		this._super(width, height);
		this.port = this.createPort("hybrid",
				new draw2d.layout.locator.CenterLocator(this));
		this.CONNECTION_DIR_STRATEGY = [$.proxy(function(conn, relatedPort) {
					return this.getParent().getBoundingBox()
							.getDirection(relatedPort.getAbsolutePosition());
				}, this.port), $.proxy(function(conn, relatedPort) {
					return this.getAbsoluteY() > relatedPort.getAbsoluteY()
							? 0
							: 2;
				}, this.port), $.proxy(function(conn, relatedPort) {
					return this.getAbsoluteX() > relatedPort.getAbsoluteX()
							? 3
							: 1;
				}, this.port)];
		...
		this.port
				.setConnectionAnchor(new draw2d.layout.anchor.ShortesPathConnectionAnchor(this.port));
	...
	repaint : function(attributes) {
		...
		if (typeof attributes.fill === "undefined") {
			if (this.bgColor !== null) {
				attributes.fill = "90-" + this.bgColor.hash() + ":5-"
						+ this.bgColor.lighter(0.3).hash() + ":95";
			} else {
	...
	getPersistentAttributes : function() {
		var memento = this._super();
		memento.dirStrategy = this.CONNECTION_DIR_STRATEGY
				.indexOf(this.port.getConnectionDirection);
		return memento;
	},
	setPersistentAttributes : function(memento) {
		this._super(memento);
...
draw2d.shape.node.HorizontalBus = draw2d.shape.node.Hub.extend({
	...
	init : function(width, height, label) {
		...
		this
				.installEditPolicy(new draw2d.policy.figure.HBusSelectionFeedbackPolicy());
	}
});
draw2d.shape.node.VerticalBus = draw2d.shape.node.Hub.extend({
	...
	init : function(width, height, label) {
		...
		this
				.installEditPolicy(new draw2d.policy.figure.VBusSelectionFeedbackPolicy());
	},
	getMinHeight : function() {
...
draw2d.shape.node.Fulcrum = draw2d.shape.node.Hub.extend({
	...
	init : function() {
		this._super(40, 40);
		this.port
				.setConnectionAnchor(new draw2d.layout.anchor.ConnectionAnchor(this.port));
		...
		this.setStroke(0);
		this
				.installEditPolicy(new draw2d.policy.figure.AntSelectionFeedbackPolicy());
	},
	repaint : function(attributes) {
		if (this.repaintBlocked === true || this.shape === null) {
			return;
...
draw2d.shape.basic.Oval = draw2d.VectorFigure.extend({
	...
	init : function(width, height, customColor) {
		this._super();
		if (customColor != null)
	...
	createShapeElement : function() {
		...
		return this.canvas.paper.ellipse(this.getAbsoluteX() + halfW, this
						.getAbsoluteY()
						+ halfH, halfW, halfH);
	},
	repaint : function(attributes) {
	...
	intersectionWithLine : function(a1, a2) {
		...
		var center = new draw2d.geo.Point(this.getAbsoluteX() + rx, this
						.getAbsoluteY()
						+ ry);
...
draw2d.shape.basic.Label = draw2d.SetFigure.extend({
	...
	init : function(text, customFontSize) {
		// is here to modify about line
		this._super();
		...
		this.fontColor = new draw2d.util.Color("#ffffff");
	...
	applyTransformation : function() {
		this.shape.transform("R" + this.rotationAngle);
		this.svgNodes.transform("R" + this.rotationAngle + "T"
				+ this.getAbsoluteX() + "," + this.getAbsoluteY());
	},
	isResizeable : function() {
		return false;
	...
	setFontFamily : function(font) {
		...
		if ((typeof font !== "undefined") && font !== null
				&& typeof this.FONT_FALLBACK[font] !== "undefined") {
	...
	getMinWidth : function() {
		...
		if (this.cachedMinWidth === null) {
			this.cachedMinWidth = this.svgNodes.getBBox(true).width + 2
					* this.padding + 2 * this.getStroke();
		}
	...
	getMinHeight : function() {
		...
		if (this.cachedMinHeight === null) {
			this.cachedMinHeight = this.svgNodes.getBBox(true).height + 2
					* this.padding + 2 * this.getStroke();
		}
...
draw2d.shape.basic.Line = draw2d.Figure.extend({
	...
	DEFAULT_COLOR : new draw2d.util.Color(255, 0, 0),
	init : function(startX, startY, endX, endY, customLineColor) {
		this.repaintBlocked = false;
		this.corona = 10;
		this.isGlowing = false;
		if (customLineColor != null)
			this.lineColor = customLineColor;
		else
	...
	createShapeElement : function() {
		return this.canvas.paper.path("M" + this.getStartX() + " "
				+ this.getStartY() + "L" + this.getEndX() + " "
				+ this.getEndY());
	},
	repaint : function(attributes) {
		if (this.repaintBlocked === true || this.shape === null) {
		...
		if (typeof attributes === "undefined") {
			attributes = {
				...
				"path" : ["M", this.getStartX(), this.getStartY(), "L",
						this.getEndX(), this.getEndY()].join(" ")
			};
		} else {
			if (typeof attributes.path === "undefined") {
				attributes.path = ["M", this.getStartX(), this.getStartY(),
	...
	getAngle : function() {
		...
		var angle = -(180 / Math.PI)
				* Math.asin((this.startY - this.endY) / length);
		if (angle < 0) {
	...
	hitTest : function(px, py) {
		return draw2d.shape.basic.Line.hit(this.corona, this.startX,
				this.startY, this.endX, this.endY, px, py);
	},
	intersection : function(other) {
		...
		segments1.each(function(i, s1) {
					segments2.each(function(j, s2) {
								var p = draw2d.shape.basic.Line.intersection(
...
draw2d.shape.basic.Line.intersection = function(a1, a2, b1, b2) {
	...
	if (u_b != 0) {
		...
		if (0 <= ua && ua <= 1 && 0 <= ub && ub <= 1) {
			result = new draw2d.geo.Point((a1.x + ua * (a2.x - a1.x)) | 0,
					(a1.y + ua * (a2.y - a1.y)) | 0);
...
draw2d.shape.basic.PolyLine = draw2d.shape.basic.Line.extend({
			...
			hitTest : function(px, py) {
				for (var i = 0; i < this.lineSegments.getSize(); i++) {
					...
					if (draw2d.shape.basic.Line.hit(this.corona, line.start.x,
							line.start.y, line.end.x, line.end.y, px, py)) {
						return true;
					}
				}
				return false;
...
draw2d.shape.basic.Image = draw2d.shape.node.Node.extend({
			...
			createShapeElement : function() {
				return this.canvas.paper.image(this.path, this.getX(), this
...
draw2d.Connection = draw2d.shape.basic.PolyLine.extend({
	...
	repaint : function(attributes) {
		...
		if (this.getSource().getParent().isMoving === false
			...
			if (this.targetDecorator !== null
					&& this.targetDecoratorNode === null) {
				this.targetDecoratorNode = this.targetDecorator.paint(this
						.getCanvas().paper);
			}
			if (this.sourceDecorator !== null
					&& this.sourceDecoratorNode === null) {
				this.sourceDecoratorNode = this.sourceDecorator.paint(this
						.getCanvas().paper);
			}
		}
		if (this.sourceDecoratorNode !== null) {
			var start = this.getPoints().get(0);
			this.sourceDecoratorNode.transform("r" + this.getStartAngle() + ","
					+ start.x + "," + start.y + " t" + start.x + "," + start.y);
			this.sourceDecoratorNode.attr({
						"stroke" : "#" + this.lineColor.hex(),
						opacity : this.alpha
					});
			this.sourceDecoratorNode.node.setAttribute("class",
					this.cssClass !== null ? this.cssClass : "");
			this.sourceDecoratorNode.forEach($.proxy(function(shape) {
						shape.node.setAttribute("class", this.cssClass !== null
										? this.cssClass
										: "");
					}, this));
		}
		if (this.targetDecoratorNode !== null) {
			...
			this.targetDecoratorNode.transform("r" + this.getEndAngle() + ","
					+ end.x + "," + end.y + " t" + end.x + "," + end.y);
	...
	getStartPoint : function() {
		if (this.isMoving === false) {
			return this.sourcePort.getConnectionAnchorLocation(this.targetPort
					.getConnectionAnchorReferencePoint());
		} else {
			return this._super();
		}
	},
	getEndPoint : function() {
		if (this.isMoving === false) {
			return this.targetPort.getConnectionAnchorLocation(this.sourcePort
					.getConnectionAnchorReferencePoint());
		} else {
			return this._super();
		}
	...
	getStartAngle : function() {
		...
		var length = Math.sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y)
				* (p1.y - p2.y));
...
draw2d.ResizeHandle = draw2d.shape.basic.Rectangle.extend({
	...
	onDragStart : function() {
		...
		this.commandMove = this.owner
				.createCommand(new draw2d.command.CommandType(draw2d.command.CommandType.MOVE));
		this.commandResize = this.owner
				.createCommand(new draw2d.command.CommandType(draw2d.command.CommandType.RESIZE));
		return true;
	},
	onDrag : function(dx, dy) {
	...
	onDragEnd : function() {
		...
		if (this.commandResize !== null) {
			this.commandResize.setDimension(this.owner.getWidth(), this.owner
							.getHeight());
	...
	setDimension : function(width, height) {
		...
		switch (this.type) {
			...
			case 9 :
				this
						.setSnapToGridAnchor(new draw2d.geo.Point(offset2,
								offset2));
				break;
		}
	},
...
draw2d.shape.basic.LineResizeHandle = draw2d.shape.basic.Circle.extend({
	...
	onDragStart : function() {
		...
		this.command = this
				.getCanvas()
				.getCurrentSelection()
				.createCommand(new draw2d.command.CommandType(draw2d.command.CommandType.MOVE_BASEPOINT));
		return true;
	},
	onDrag : function(dx, dy) {
		this.setPosition(this.ox + dx, this.oy + dy);
		var port = this.getOppositePort();
		var target = port !== null ? port.getDropTarget(this.getX(), this
						.getY(), null) : null;
...
draw2d.shape.basic.LineStartResizeHandle = draw2d.shape.basic.LineResizeHandle
		.extend({
					...
					onDrag : function(dx, dy) {
						...
						this.owner.setStartPoint(objPos.x - diffX, objPos.y
										- diffY);
					...
					onDrop : function(dropTarget) {
						...
						if (this.owner instanceof draw2d.Connection
								&& this.command !== null) {
							this.command.setNewPorts(dropTarget, this.owner
											.getTarget());
							this.getCanvas().getCommandStack()
									.execute(this.command);
						}
						this.command = null;
					}
				});
draw2d.shape.basic.LineEndResizeHandle = draw2d.shape.basic.LineResizeHandle
		.extend({
					...
					onDrag : function(dx, dy) {
						...
						this.owner.setEndPoint(objPos.x - diffX, objPos.y
										- diffY);
						this.owner.isMoving = true;
					...
					onDrop : function(dropTarget) {
						...
						if (this.owner instanceof draw2d.Connection
								&& this.command !== null) {
							this.command.setNewPorts(this.owner.getSource(),
									dropTarget);
							this.getCanvas().getCommandStack()
									.execute(this.command);
						}
...
draw2d.shape.basic.JunctionResizeHandle = draw2d.shape.basic.LineResizeHandle
		.extend({
...
draw2d.Port = draw2d.shape.basic.Circle.extend({
	...
	init : function(name) {
		...
		this
				.installEditPolicy(new draw2d.policy.port.IntrusivePortsFeedbackPolicy());
	},
	...
	getConnectionDirection : function(conn, relatedPort) {
		return this.getParent().getBoundingBox().getDirection(this
				.getAbsolutePosition());
	},
	...
	repaint : function(attributes) {
		...
		if (this.getAlpha() < 0.9) {
			...
			attributes.fill = ["90", this.bgColor.hash(), this.lighterBgColor]
					.join("-");
		}
		this._super(attributes);
	},
	...
	onDrag : function(dx, dy) {
		this.isInDragDrop = true;
		this._super(dx, dy);
		var target = this.getDropTarget(this.getAbsoluteX(), this
						.getAbsoluteY(), this);
		if (target !== this.currentTarget) {
			if (this.currentTarget !== null) {
				...
				this.editPolicy.each($.proxy(function(i, e) {
							if (e instanceof draw2d.policy.port.PortFeedbackPolicy) {
								e.onHoverLeave(this.canvas, this,
										this.currentTarget);
							}
						}, this));
			}
			if (target !== null) {
				...
				if (this.currentTarget !== null) {
					...
					this.editPolicy.each($.proxy(function(i, e) {
						if (e instanceof draw2d.policy.port.PortFeedbackPolicy) {
							e.onHoverEnter(this.canvas, this,
									this.currentTarget);
						}
					}, this));
				}
	...
	hitTest : function(iX, iY) {
		var x = this.getAbsoluteX() - (this.coronaWidth * 2) - this.getWidth()
				/ 2;
		var y = this.getAbsoluteY() - (this.coronaWidth * 2) - this.getHeight()
				/ 2;
	...
	setGlow : function(flag) {
		if (flag === true && this.corona === null) {
			...
			this.corona.setDimension(this.getWidth()
							+ (this.getCoronaWidth() * 2), this.getWidth()
							+ (this.getCoronaWidth() * 2));
			this.corona.setPosition(this.getAbsoluteX() - this.getCoronaWidth()
							- this.getWidth() / 2, this.getAbsoluteY()
							- this.getCoronaWidth() - this.getHeight() / 2);
...
draw2d.InputPort = draw2d.Port.extend({
			...
			createCommand : function(request) {
				if (request.getPolicy() === draw2d.command.CommandType.CONNECT) {
					if (request.source.getParent().getId() === request.target
							.getParent().getId()) {
						return null;
					}
					if (request.source instanceof draw2d.OutputPort) {
						return new draw2d.command.CommandConnect(
								request.canvas, request.source, request.target,
								request.source);
					}
			...
			createCommand : function(request) {
				if (request.getPolicy() === draw2d.command.CommandType.CONNECT) {
					...
					if (request.source instanceof draw2d.InputPort) {
						return new draw2d.command.CommandConnect(
								request.canvas, request.target, request.source,
								request.source);
					}
					if (request.source instanceof draw2d.HybridPort) {
						return new draw2d.command.CommandConnect(
			...
			createCommand : function(request) {
				if (request.getPolicy() === draw2d.command.CommandType.CONNECT) {
					...
					if (request.source instanceof draw2d.InputPort) {
						...
						if (request.source instanceof draw2d.OutputPort) {
							return new draw2d.command.CommandConnect(
									request.canvas, request.source,
									request.target, request.source);
						} else {
							if (request.source instanceof draw2d.HybridPort) {
								return new draw2d.command.CommandConnect(
										request.canvas, request.target,
...
draw2d.layout.anchor.ChopboxConnectionAnchor = draw2d.layout.anchor.ConnectionAnchor
		.extend({
			...
			getLocation : function(reference) {
				...
				var scale = 0.5
						/ Math.max(Math.abs(dx) / r.w, Math.abs(dy) / r.h);
				dx *= scale;
				dy *= scale;
		...
		.extend({
			...
			getLocation : function(ref) {
				...
				if (this.getOwner().getParent() instanceof draw2d.shape.basic.Oval) {
					var result = this.getOwner().getParent()
							.intersectionWithLine(ref, center);
					if (result.getSize() == 1) {
						return result.get(0);
					} else {
						console.log("error");
				...
				var octant = r.determineOctant(new draw2d.geo.Rectangle(ref.x,
						ref.y, 2, 2));
				switch (octant) {
					case 0 :
						return r.getTopLeft();
...
draw2d.shape.widget.Slider = draw2d.shape.widget.Widget.extend({
			...
			setDimension : function(w, h) {
				this._super(w, h);
				this.slideBoundingBox.setBoundary(0, 0, this.getWidth() - 10,
						this.getHeight());
			...
			onPanning : function(dx, dy) {
				this.slideBoundingBox.setPosition(this.origX + dx, this.origY
								+ dy);
				this.setValue(100
						/ (this.slideBoundingBox.bw - this.slideBoundingBox
								.getWidth()) * this.slideBoundingBox.getX());
			},
			setValue : function(value) {
			...
			repaint : function(attributes) {
				...
				var thumbX = ((this.slideBoundingBox.bw - this.slideBoundingBox
						.getWidth())
			...
			applyTransformation : function() {
				this.svgNodes.transform("T" + this.getAbsoluteX() + ","
						+ this.getAbsoluteY());
			}
		});
draw2d.shape.diagram.Diagram = draw2d.SetFigure.extend({
			...
			applyTransformation : function() {
				if (this.isResizeable() === true) {
					this.svgNodes.transform("S" + this.scaleX + ","
							+ this.scaleY + "," + this.getAbsoluteX() + ","
							+ this.getAbsoluteY() + "t" + this.getAbsoluteX()
							+ "," + this.getAbsoluteY());
				} else {
					this.svgNodes.transform("T" + this.getAbsoluteX() + ","
							+ this.getAbsoluteY());
...
draw2d.shape.diagram.Sparkline = draw2d.shape.diagram.Diagram.extend({
	...
	repaint : function(attributes) {
		...
		if (this.svgNodes !== null
				&& (typeof this.cache.pathString === "undefined")) {
			var prev_pt = null;
			$.each(this.data, $.proxy(function(idx, item) {
						...
						if (prev_pt === null) {
							...
							this.cache.pathString = [this.cache.pathString,
									"L", pt.x, pt.y].join(" ");
						}
						prev_pt = pt;
					}, this));
...
draw2d.shape.analog.ResistorVertical = draw2d.SetFigure.extend({
	...
	createSet : function() {
		var set = this._super();
		set
				.push(this.canvas.paper
						.path("M15,0 L15,5 L0,7.5 L30,10 L0,15 L30,20 L0,25 L30,30 L15,32.5 L15,40"));
		return set;
	}
});
...
draw2d.shape.layout.HorizontalLayout = draw2d.shape.layout.Layout.extend({
			...
			getMinWidth : function() {
				var width = this.stroke * 2
						+ Math.max(0, this.children.getSize() - 1) * this.gap;
				this.children.each(function(i, e) {
			...
			setDimension : function(w, h) {
				...
				if (diff > 0) {
					...
					this.children.each(function(i, e) {
								e.figure.setDimension(e.figure.getMinWidth()
												+ diff, e.figure.getHeight());
							});
				} else {
					this.children.each(function(i, e) {
			...
			applyTransformation : function() {
				if (this.isResizeable() === true) {
					this.svgNodes.transform("S" + this.scaleX + ","
							+ this.scaleY + "," + this.getAbsoluteX() + ","
							+ this.getAbsoluteY() + "t"
							+ (this.getAbsoluteX() - this.offsetX) + ","
							+ (this.getAbsoluteY() - this.offsetY));
				} else {
					this.svgNodes.transform("T"
							+ (this.getAbsoluteX() - this.offsetX) + ","
							+ (this.getAbsoluteY() - this.offsetY));
				}
			},
...
draw2d.shape.icon.ArrowLeft = draw2d.shape.icon.Icon.extend({
	...
	createSet : function() {
		return this.canvas.paper
				.path("M24.316,5.318L6.684,15.5l17.632,10.182V5.318L24.316,5.318z");
	}
});
draw2d.shape.icon.ArrowUp = draw2d.shape.icon.Icon.extend({
			...
			createSet : function() {
				return this.canvas.paper
						.path("M25.682,24.316L15.5,6.684L5.318,24.316H25.682z");
			}
		});
draw2d.shape.icon.ArrowDown = draw2d.shape.icon.Icon.extend({
			...
			createSet : function() {
				return this.canvas.paper
						.path("M5.318,6.684L15.5,24.316L25.682,6.684H5.318z");
			}
		});
...
draw2d.shape.pert.Activity = draw2d.shape.layout.VerticalLayout.extend({
			...
			init : function() {
				...
				top.setStroke(0);
				this.earlyStartLabel = this.createLabel("Early Start")
						.setStroke(0);
				...
				this.activityLabel
						.installEditor(new draw2d.ui.LabelInplaceEditor());
				...
				bottom.setStroke(0);
				this.lateStartLabel = this.createLabel("Late Start")
						.setStroke(0);
			...
			getEarlyStart : function() {
				...
				this.inputPort.getConnections().each(function(i, conn) {
					...
					latestEarlyEnd = Math.max(latestEarlyEnd, parentActivity
									.getEarlyEnd());
				});
			...
			setLateFinish : function(value) {
				...
				this.stackLabel.setText(lateStart
						- parseFloat(this.earlyStartLabel.getText()));
			...
			repaint : function(attributes) {
				...
				if (this.getAlpha() < 0.9) {
					...
					attributes.fill = ["90", this.bgColor.hash(),
							this.lighterBgColor].join("-");
				}
				this._super(attributes);
			}
		});
draw2d.shape.state.Start = draw2d.shape.basic.Circle.extend({
	...
	init : function() {
		this._super();
		this.port = this.createPort("output",
...
draw2d.shape.state.End = draw2d.shape.basic.Circle.extend({
	...
	init : function() {
		...
		this._super();
		this.port = this.createPort("input",
		...
		this.addFigure(this.innerCircle,
				new draw2d.layout.locator.CenterLocator(this));
		this.setStroke(0);
	},
	setDimension : function(w, h) {
		this._super(w, h);
		this.innerCircle.setDimension(this.getWidth() - 10, this.getHeight()
						- 10);
	}
});
draw2d.shape.state.State = draw2d.shape.layout.VerticalLayout.extend({
	...
	init : function() {
		...
		this.port = this.createPort("hybrid",
				new draw2d.layout.locator.BottomLocator(this));
		this.port
				.setConnectionAnchor(new draw2d.layout.anchor.ChopboxConnectionAnchor(this.port));
		this.setBackgroundColor("#f3f3f3");
		this.setStroke(1);
...
draw2d.shape.state.Connection = draw2d.Connection.extend({
	...
	init : function() {
		...
		this.addFigure(this.label,
				new draw2d.layout.locator.PolylineMidpointLocator(this));
	},
	setLabel : function(text) {
		this.label.setText(text);
...
draw2d.ui.LabelInplaceEditor = draw2d.ui.LabelEditor.extend({
			...
			start : function(label) {
				...
				bb.setPosition(canvas
						.fromCanvasToDocumentCoordinate(bb.x, bb.y));
...
draw2d.decoration.connection.ArrowDecorator = draw2d.decoration.connection.Decorator
		.extend({
					...
					paint : function(paper) {
						var st = paper.set();
						st.push(paper.path(["M0 0", "L", this.width, " ",
								-this.height / 2, "L", this.width, " ",
								this.height / 2, "L0 0"].join("")));
						st.attr({
...
draw2d.decoration.connection.DiamondDecorator = draw2d.decoration.connection.Decorator
		.extend({
					...
					paint : function(paper) {
						var st = paper.set();
						st.push(paper.path(["M", this.width / 2, " ",
								-this.height / 2, "L", this.width, " ", 0, "L",
								this.width / 2, " ", this.height / 2, "L", 0,
								" ", 0, "L", this.width / 2, " ",
...
draw2d.decoration.connection.CircleDecorator = draw2d.decoration.connection.Decorator
		.extend({
...
draw2d.decoration.connection.BarDecorator = draw2d.decoration.connection.Decorator
		.extend({
...
draw2d.io.svg.Writer = draw2d.io.Writer.extend({
	...
	marshal : function(canvas) {
		...
		var svg = canvas.getHtmlContainer().html().replace(/>\s+/g, ">")
				.replace(/\s+</g, "<");
		svg = this.formatXml(svg);
		svg = svg
				.replace(/<desc>.*<\/desc>/g,
						"<desc>Create with draw2d JS graph library and RaphaelJS</desc>");
...
(function() {
	Raphael.fn.group = function(f, g) {
		...
		this.group.translate = function(c, a) {
			this.setAttribute("transform", "translate(" + c + "," + a
							+ ") scale(" + this.getAttr("scale").x + ","
							+ this.getAttr("scale").y + ")");
		};
		this.group.rotate = function(c, a, e) {
			this.setAttribute("transform", "translate("
							+ this.getAttr("translate").x + ","
							+ this.getAttr("translate").y + ") scale("
							+ this.getAttr("scale").x + ","
							+ this.getAttr("scale").y + ") rotate(" + c + ","
							+ a + "," + e + ")");
		};
		this.group.scale = function(c, a) {
			this.setAttribute("transform", "scale(" + c + "," + a
							+ ") translate(" + this.getAttr("translate").x
							+ "," + this.getAttr("translate").y + ")");
		};
		this.group.push = function(c) {
			this.appendChild(c.node);
		};
		this.group.getAttr = function(c) {
			this.previous = this.getAttribute("transform") ? this
