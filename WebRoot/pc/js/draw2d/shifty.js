/**
 * @license Shifty <http://jeremyckahn.github.com/shifty/> Author: Jeremy Kahn -
 *          jeremyckahn@gmail.com License: MIT Version: 0.6.1 (Mon, 16 Apr 2012
 *          16:27:31 GMT)
 */
...
(function() {
	...
	(function(global) {
		...
		if (typeof SHIFTY_DEBUG_NOW === 'function') {
			...
			/**
			 * Get the current UNIX epoch time as an integer. Exposed publicly
			 * as `Tweenable.util.now()`.
			 * 
			 * @return {Number} An integer representing the current timestamp.
			 */
			now = function() {
				return +new Date();
			};
		 ...
		 * `hasOwnProperty` wizardry for you. This is also exposed publicly, you
		 * can access it as `Tweenable.util.each()`.
		 * 
		 * @param {Object}
		 *            obj The object to iterate through.
		 * @param {Function}
		 *            func The function to pass the object and "own" property
		 *            to. This handler function receives the `obj` back as the
		 *            first parameter, and a property name as the second.
		 */
		...
		/**
		 * Does a basic copy of one Object's properties to another. This is not
		 * a robust `extend` function, nor is it recusrsive. It is only
		 * appropriate to use on objects that have primitive properties
		 * (Numbers, Strings, Boolean, etc.). Exposed publicly as
		 * `Tweenable.util.simpleCopy()`
		 * 
		 * @param {Object}
		 *            targetObject The object to copy into
		 * @param {Object}
		 *            srcObject The object to copy from
		...
		/**
		 * Calculates the interpolated tween values of an Object based on the
		 * current time.
		 * 
		 * @param {Number}
		 *            currentPosition The current position to evaluate the tween
		 *            against.
		 * @param {Object}
		 *            params A configuration Object containing the values that
		 *            this function requires. The required properties in this
		 *            Object are:
		 * @property {Object} originalState The original properties the Object
		 *           is tweening from.
		 * @property {Object} to The destination properties the Object is
		 *           tweening to.
		 ...
		 * @param {Object}
		 *            state A configuration object containing current state data
		 ...
		 * 
		 * @param {number}
		 *            from The origination value of the tween.
		 * @param {number}
		 *            to The destination value of the tween.
		 * @param {Function}
		 *            easingFunc The easing formula to apply to the tween.
		 * @param {number}
		 *            position The normalized position (between 0.0 and 1.0) to
		 ...
		 * @param {number}
		 *            fps The fraction of a second in the update should occur.
		 ...
		 * 
		 * @param {String}
		 *            hookName The name of the hook to invoke the handlers of.
		 * @param {Object}
		 *            hooks The object containing the hook Arrays.
		 * @param {Object}
		 *            applyTo The `Tweenable` instance to call the hooks upon.
		 * @param {Array}
		 *            args The arguments to pass to each function in the
		 *            specified hook.
		 */
		 ...
		 * @param {Object}
		 *            applyTo The `Tweenable` instance to call the filter upon.
		 * @param {Array}
		 *            args The arguments to pass to the function in the
		 *            specified filter.
		 */
		 ...
		 * 
		 * @param {Object}
		 *            params The configuration containing all of a tween's
		 *            properties. This requires all of the `params`
		 * @properties required for `tweenProps`, so see that. It also requires:
		 ...
		 *           belong to `owner
		 * @param {Object}
		 *            state The configuration Object containing all of the state
		 *            properties for a `Tweenable` instance. It requires all of
		 *            the
		...
		function timeoutHandler(params, state) {
			var endTime = params.timestamp + params.duration, currentTime = Math
					.min(now(), endTime), didEnded = currentTime >= endTime;
			...
			if (state.isTweening) {
				...
				if (params.hook.step) {
					invokeHook('step', params.hook, params.owner,
							[state.current]);
				}
		 ...
		 * easing Object. If `easing` is an Object, then this function clones it
		 * and fills in the missing properties with "linear".
		 * 
		 * @param {Object}
		 *            fromTweenParams
		 * @param {Object|string}
		 ...
		 * 
		 * @codestart var tweenableInst = new Tweenable({});
		 * @codeend
		 * 
		 * It accepts one parameter:
		 * 
		 * @param {Object}
		 *            options A configuration Object containing options for the
		 *            `Tweenable` instance. The following are valid:
		 * @property {Object} initialState The state at which the first tween
		 *           should begin at.
		 * @property {Number} duration The default `duration` for each `tween`
		 *           made by this instance. Default is 500 milliseconds.
		 * @property {Number} fps The frame rate (frames per second) at which
		 *           the instance will update. Default is 30.
		 * @property {String} easing The name of the default easing formula
		 *           (attached to `Tweenable.prototype.formula`) to use for each
		 *           `tween` made for this instance. Default is `linear`.
		...
		Tweenable = function(options) {
			...
			// The framerate at which Shifty updates. This is exposed publicly
			// as
			// `tweenableInst.fps`.
		 ...
		 * or the longhand way:
		 * 
		 * tweenableInst.tween ( { from: Object, to: Object, duration: Number,
		 * callback: Function, easing: String|Object, step: Function });
		 * 
		 * Regardless of how you invoke this method, the only required
		 * parameters are `from` and `to`.
		 * 
		 * @param {Object}
		 *            from The beginning state Object containing the properties
		 *            to tween from. NOTE: The properties of this Object are
		 *            modified over time (to reflect the values in `to`).
		 * @param {Object}
		 *            to The target state Object containing the properties to
		 *            tween to.
		 * @param {Number}
		 *            duration The amount of time in milliseconds that the tween
		 *            should run for.
		 * @param {Function}
		 *            start The function to be invoked as soon as the this tween
		 *            starts. Mostly useful when used with the `queue`
		 *            extension.
		 * @param {Function}
		 *            callback The function to invoke as soon as this tween
		 *            completes. This function is given the tween's current
		 *            state Object as the first parameter.
		 * @param {String|Object}
		 *            easing This can either be a string specifying the easing
		 *            formula to be used on every property of the tween, or an
		 *            Object with values that are strings that specify an easing
		 *            formula for a specific property. Any properties that do
		 *            not have an easing formula specified will use "linear".
		 * @param {Function}
		 *            step A function to call for each step of the tween. A
		 *            "step" is defined as one update cycle (frame) of the
		 *            tween. Many update cycles occur to create the illusion of
		 *            motion, so this function will likely be called quite a
		 *            bit.
		 */
		Tweenable.prototype.tween = function(from, to, duration, callback,
				easing) {

			var self, params, state;
			...
			// Normalize some internal values depending on how
			// `tweenableInst.tween`
			// was invoked
			if (to) {
				...
				// If the second argument is not present, assume the longhand
				// syntax is
				// being used.
		...
		/**
		 * Convenience method for tweening from the current position. This
		 * method functions identically to `tween()` (it's just a wrapper
		 * function), but implicitly passes the `Tweenable` instance's current
		 * state (what you get from `get()`) as the `from` parameter. This
		 * supports both the longhand and shorthand syntax that `tween()` does,
		 * just omitting the `from` paramter in both cases.
		 * 
		 * @param {Object}
		 *            target If the other parameters are omitted, this Object
		 *            should contain the longhand parameters outlined in
		 *            `tween()`. If at least one other formal parameter is
		 *            specified, then this Object should contain the target
		 *            values to tween to.
		 * @param {Number}
		 *            duration Duration of the tween, in milliseconds.
		 * @param {Function}
		 *            callback The callback function to pass along to `tween()`.
		 * @param {String|Object}
		 *            easing The easing formula to use.
		 */
		...
		/**
		 * Returns a reference to the `Tweenable`'s current state (the `from`
		 * Object that wat passed to `tweenableInst.tween()`).
		 * 
		 * @return {Object}
		 */
		 ...
		 * 
		 * @param {Object}
		 *            state The state the instance shall have.
		 */
		 ...
		 * 
		 * @param {Boolean}
		 *            gotoEnd If `false`, or omitted, the tween just stops at
		 *            its current state, and the `callback` is not invoked. If
		 *            `true`, the tweened object's values are instantly set the
		 *            the target "to" values, and the `callback` is invoked.
		...
		Tweenable.prototype.stop = function(gotoEnd) {
			...
			if (gotoEnd) {
				...
				applyFilter('afterTweenEnd', this, [this._state.current,
								this._tweenParams.originalState,
								this._tweenParams.to]);
		 ...
		 * invoked at key points in a `Tweenable` instance's lifecycle. A hook
		 * that is related to the tweening process (like `step`), for example,
		 * will occur for every tween that is performed by the `Tweenable`
		 * instance. You just have to set it once. You can attach as many
		 * functions to any given hook as you like. The available hooks are as
		 * follows:
		 *  - `step`: Runs on every frame that a tween runs for. Hook handler
		 * function receives a tween's `currentState` for a parameter.
		 * 
		 * @param {String}
		 *            hookName The name of the hook to attach `hookFunc` to.
		 * @param {Function}
		 *            hookFunc The hook handler function. This function will
		 *            receive parameters based on what hook it is being attached
		 *            to.
		 */
		 ...
		 * 
		 * @param {String}
		 *            hookName The hook to remove a function or functions from.
		 * @param {String|undefined}
		 *            hookFunc The function to matched against and remove from
		 *            the hook handler list. If omitted, all functions are
		...
		/**
		 * Globally exposed static property to attach filters to. Filters are
		 * used for transforming the properties of a tween at various points in
		 * a `Tweenable` instance's lifecycle. Please consult the README for
		 * more info on this.
		 */
		Tweenable.prototype.filter = {};

		/**
		 * Globally exposed static helper methods. These methods are used
		 * internally and could be helpful in a global context as well.
		 */
		Tweenable.util = {
			'now' : now,
			'each' : each,
		 ...
		 * extendable - simply attach properties to the
		 * Tweenable.prototype.formula Object following the same format at
		 * `linear`.
		 */
	...
	/**
	 * Shifty Easing Formulas Adapted for Shifty by Jeremy Kahn -
	 * jeremyckahn@gmail.com v0.1.0
	 * 
	 * ================================ All equations are adapted from Thomas
	 * Fuchs' Scripty2:
	 ...
	 * https://github.com/jeremyckahn/shifty/blob/master/README.md
	 * 
		...
		Tweenable.util.simpleCopy(Tweenable.prototype.formula, {
			...
			bouncePast : function(pos) {
				if (pos < (1 / 2.75)) {
					...
					return 2
							- (7.5625 * (pos -= (2.625 / 2.75)) * pos + .984375);
				}
			},
	...
	/**
	 * Shifty Queue Extension By Jeremy Kahn - jeremyckahn@gmail.com v0.2.0
	 * 
	 * Dependencies: shifty.core.js
	 * 
	 * Tweeny and all official extensions are freely available under an MIT
	 * license. For instructions on how to use Tweeny and this extension, please
	 * consult the manual:
	 * https://github.com/jeremyckahn/shifty/blob/master/README.md For
	 * instructions on how to use this extension, please see:
	 * https://github.com/jeremyckahn/shifty/blob/master/doc/shifty.queue.md
	 * 
	 * MIT Lincense. This code free to use, modify, distribute and enjoy.
	 * 
	 */
	...
	(function() {
		...
		function tweenInit(context, from, to, duration, callback, easing) {
			// Duck typing! This method infers some info from the parameters
			// above to determine which method to call,
			...
			return function() {
				if (to) {
					...
					// Ensure that that `wrappedCallback` (from `queue`) gets
					// passed along.
		...
		Tweenable.prototype.queue = function(from, to, duration, callback,
				easing) {
			var wrappedCallback;
			...
			this._tweenQueue.push(tweenInit(this, from, to, duration,
					wrappedCallback, easing));
	...
	/**
	 * Shifty Color Extension By Jeremy Kahn - jeremyckahn@gmail.com v0.1.0
	 * 
	 * For instructions on how to use Shifty, please consult the README:
	 * https://github.com/jeremyckahn/shifty/blob/master/README.md For
	 * instructions on how to use this extension, please see:
	 * https://github.com/jeremyckahn/shifty/blob/master/doc/shifty.color.md
	 * 
	 * MIT Lincense. This code free to use, modify, distribute and enjoy.
	 * 
	 */
	...
	(function() {
		...
		var R_SHORTHAND_HEX = /^#([0-9]|[a-f]){3}$/i, R_LONGHAND_HEX = /^#([0-9]|[a-f]){6}$/i, R_RGB = /^rgb\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\)\s*$/i, savedRGBPropNames;
		...
		/**
		 * Convert a hexadecimal string to an array with three items, one each
		 * for the red, blue, and green decimal values.
		 * 
		 * @param {String}
		...
		function hexToRGBArr(hex) {
			...
			// If the string is a shorthand three digit hex notation, normalize
			// it to the standard six digit notation
			if (hex.length === 3) {
				hex = hex.split('');
		...
		function getRGBStringFromHex(str) {
			var rgbArr, convertedStr;
			rgbArr = hexToRGBArr(str);
			convertedStr = 'rgb(' + rgbArr[0] + ',' + rgbArr[1] + ','
					+ rgbArr[2] + ')';
		...
		function isColorString(str) {
			return (typeof str === 'string')
					&& (R_SHORTHAND_HEX.test(str) || R_LONGHAND_HEX.test(str) || R_RGB
							.test(str));
		}

		function isHexString(str) {
			return (typeof str === 'string')
					&& (R_SHORTHAND_HEX.test(str) || R_LONGHAND_HEX.test(str));
		}
		...
		Tweenable.prototype.filter.color = {
			...
			'afterTween' : function(currentState, fromState, toState) {
				...
				collapseEasingObject(this._tweenParams.easing,
						savedRGBPropNames);
			}
		};
	...
	/**
	 * Shifty CSS Unit Extension By Jeremy Kahn - jeremyckahn@gmail.com v0.1.0
	 * 
	 * For instructions on how to use Shifty, please consult the README:
	 * https://github.com/jeremyckahn/shifty/blob/master/README.md For
	 * instructions on how to use this extension, please see:
	 * https://github.com/jeremyckahn/shifty/blob/master/doc/shifty.css_units.md
	 * 
	 * MIT Lincense. This code free to use, modify, distribute and enjoy.
	 * 
	 */
	...
	(function() {
		...
		function deTokenize(obj, tokenProps) {
			Tweenable.util.each(tokenProps, function(collection, token) {
						...
						if (currentTokenChunks[0] === '') {
							...
							rChunkFinder = new RegExp('(' + firstChunk + '|'
											+ secondChunk + ')', 'g');
						}
		...
		function reTokenize(obj, tokenProps) {
			Tweenable.util.each(tokenProps, function(collection, token) {
						...
						obj[token] = tokenChunks[0] + obj[token]
								+ tokenChunks[1];
					});
		}
	...
	/**
	 * Shifty Interpolate Extension By Jeremy Kahn - jeremyckahn@gmail.com
	 * v0.1.0
	 * 
	 * Dependencies: shifty.core.js
	 * 
	...
	(function() {
		...
		Tweenable.util.interpolate = function(from, to, position, easing) {
			var current, interpolatedValues, mockTweenable;
			...
			Tweenable.util.applyFilter('tweenCreated', mockTweenable, [current,
							from, to]);
			Tweenable.util.applyFilter('beforeTween', mockTweenable, [current,
							from, to]);
			interpolatedValues = getInterpolatedValues(from, current, to,
					position, mockTweenable._tweenParams.easing);
		...
		Tweenable.prototype.interpolate = function(to, position, easing) {
			...
			interpolatedValues = Tweenable.util.interpolate(this.get(), to,
					position, easing);
	...
	/**
	 * Shifty Rounding Extension By Jeremy Kahn - jeremyckahn@gmail.com v0.1.0
	 * 
	 * MIT Lincense. This code free to use, modify, distribute and enjoy.
	 * 
	 */
