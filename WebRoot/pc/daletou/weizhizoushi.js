...
drawWhzs();
	...
	for(var i=0;i<1;i++){
		...
		// 1号球尾数
		for (var j = 0; j < 10; j++) { // 列数
			var $td = $("<td></td>").toggleClass("one1");
				$td.text(j);
				$td.css("background-color", "#F7F0F0");
				$td.css("color", "#F7F0F0"); // 字体颜色
				$tr.append($td);
			}
			
			// 2号球尾数
			for (var j = 0; j < 10; j++) { // 列数
				var $td = $("<td></td>").toggleClass("one2");;
					$td.css("color", "#EAEEEE"); // 字体颜色
					$td.text(j);
				$tr.append($td);
			}
