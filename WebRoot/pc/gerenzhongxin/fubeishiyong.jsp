	...
	<table class="table table-border table-bordered table-bg table-sort">
		<thead>
			...
			<tr class="text-c">
				...
				<th width="100">竞猜金额</th>	
				<th width="90">竞猜期数</th>	
				<th width="150">竞猜时间</th>
				<th width="200">竞猜订单号</th>
				<th width="90">操作</th>
			</tr>
		</thead>
		<tbody>

		<c:forEach items="${shiyongmap.list}" var="cz" varStatus="i" >
			<tr class="text-c">
				<td>${i.index+1}</td>
				<td>${cz.userinfo.usersName}</td>
				<td>${cz.userinfo.usersPhone}</td>
				<td><span class="label label-danger radius">
				<fmt:formatNumber value="${cz.jcFubei}" pattern="0.00"></fmt:formatNumber>元</span>
				</td>
				<td>${cz.jcQishu}</td>	
				<td><fmt:formatDate value="${cz.jcTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>${cz.jcNumber}</td>	
				<td>
				<a title="详情"  onclick="ckxiangqing('中奖详情','${pageContext.request.contextPath}/shiyong.do?p=FindByIdzhongjiangxiangqing&id='+'${cz.jcId}','${cz.jcId}','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">查看详情 </i></a> 
				</td>
			</tr>
			</c:forEach>
