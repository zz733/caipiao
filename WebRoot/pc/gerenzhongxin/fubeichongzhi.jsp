...
<form action="${pageContext.request.contextPath}/rechargerecord.do?p=Allchongzhirecord" method="post" name="f" id="f"  autocomplete="off">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"  > <span class="l"></span> <span class="r">共有数据：<strong>
		<c:choose>
			<c:when test="${empty chongzhirecordmap.sum}">
				0
			</c:when>
			<c:when test="${not empty chongzhirecordmap.sum}">
				${chongzhirecordmap.sum}
			</c:when>
		</c:choose>
	</strong> 条</span> 
	</div>
	...
	<table class="table table-border table-bordered table-bg table-sort">
		<thead>
			<tr>
				<th scope="col" colspan="8">使用记录</th>
			</tr>
			<tr class="text-c">
		...
		<c:forEach items="${chongzhirecordmap.list}" var="cz" varStatus="i" >
			<tr class="text-c">
				<td>${i.index+1}</td>
				<td>${cz.userinfo.usersName}</td>
				<td>${cz.userinfo.usersPhone}</td>
				<td><span class="label label-danger radius">
				<fmt:formatNumber value="${cz.rrMoney}" pattern="0.00"></fmt:formatNumber>元</span>
				</td>
